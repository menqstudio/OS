...
| **F-02 / F-18** static evidence facts | ◑ **PARTIAL (2026-08-06)** | Now per-run: `receipt_id` (supervisor-minted, `UNIQUE`), and `record_handle` / `lease_handle` / `execution_receipt_handle` — the supervisor BUILDS a governed-turn record + execution receipt from its own rows, addresses the exact persisted lease bytes, and publishes all three to the protected store; they left `produced` entirely, and the live kit's placeholder blobs are deleted. `containment_evidence_handle` is now written per run by the RECORDER (`--containment-out`: pinned image digests, lease digest, cgroup, the §2.7 fd contract, invoker uid/gid, launcher exit) and content-addressed by the broker; a missing report is a refusal. **Still open:** the four `evidence_*` counters — nothing measures a real recorder evidence chain, and they are now the ONLY static values left in the live kit's `facts` block. |
...
| **F-10** §2.5 TCB integrity floor has no caller | ◑ **PARTIAL (2026-08-06)** | The floor now has a REAL caller and a REAL probe: `brops-broker::tcb_probe::LinuxFsProbe` stats through `O_PATH|O_NOFOLLOW` and digests contents, and `build_governed_executor` runs `verify_tcb_integrity` BEFORE it will serve any governed turn — no manifest configured, unreadable, malformed, or any violation ⇒ it keeps `UpstreamBlockedExecutor`. Four tests pin the refusal branches. **Still open:** the live kit does not yet PROVISION the full 22-artifact pinned set (per-service configs, IPC policies, unit files), so the floor is enforcing on the production broker path but the proof kit has nothing to present to it. |
