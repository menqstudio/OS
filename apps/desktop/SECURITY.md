...
_Last updated: 2026-08-14. Primary platform: Linux (Debian); also built/tested on Windows. Reflects the original 10 rounds of BroPS remediation **plus** the OS-monorepo governed-execution hardening (Waves 1–3a complete; Wave 3b in progress). **No production "Verified" — `trusted_verified` is unreachable and every governed turn is refused rather than faked.** That sentence is the durable one; the wave's exact position is not restated here, because this line said "Wave 3b-1 in progress on PR #31/#32 — not merged" from 2026-07-27 until today, long after both were superseded (the work landed via PR #48 and the pull requests after it). **For where the wave actually is, read [`NEXT_CHAT.md`](../../NEXT_CHAT.md) and [`config/current_state.json`](../../config/current_state.json)** — the second is checked against live GitHub by `tools/check_repo_state.py` and cannot quietly drift; this file can, and did._
