...
//!
//! Applies every migration (0001..0014) in order to a fresh SQLite database and
//! asserts the resulting shape: `SCHEMA_VERSION == 14`, the ledger is contiguous
//! 1..=14, the key tables/columns exist, and the constraints introduced across
...
fn migrate_applies_all_versions_in_order_to_a_fresh_db() {
    ...
    assert_eq!(db::SCHEMA_VERSION, 14, "crate SCHEMA_VERSION must be 14");
    assert_eq!(db::current_version(&c).unwrap(), 14);
    // The ledger is contiguous 1..=14 — nothing skipped, nothing double-counted.
    assert_eq!(applied_versions(&c), (1..=14).collect::<Vec<i64>>());
}
...
fn migrate_is_idempotent_across_a_real_file_reopen() {
...

    {
        ...
        assert_eq!(db::current_version(&c1).unwrap(), 14);
    } // c1 dropped

    let c2 = db::open(path).unwrap(); // reopen re-runs migrate() (idempotent)
    assert_eq!(db::current_version(&c2).unwrap(), 14);
    assert_eq!(applied_versions(&c2), (1..=14).collect::<Vec<i64>>());
}
