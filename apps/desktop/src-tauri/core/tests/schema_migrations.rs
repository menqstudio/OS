...
fn migrate_applies_all_versions_in_order_to_a_fresh_db() {
    ...
    assert_eq!(db::SCHEMA_VERSION, 17, "crate SCHEMA_VERSION must be 17");
    assert_eq!(db::current_version(&c).unwrap(), 17);
    // The ledger is contiguous 1..=17 — nothing skipped, nothing double-counted.
    assert_eq!(applied_versions(&c), (1..=17).collect::<Vec<i64>>());
...
fn migrate_is_idempotent_across_a_real_file_reopen() {
...

    {
        ...
        assert_eq!(db::current_version(&c1).unwrap(), 17);
    } // c1 dropped

    let c2 = db::open(path).unwrap(); // reopen re-runs migrate() (idempotent)
    assert_eq!(db::current_version(&c2).unwrap(), 17);
    assert_eq!(applied_versions(&c2), (1..=17).collect::<Vec<i64>>());
}
