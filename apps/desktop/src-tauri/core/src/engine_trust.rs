...
//!
//! It used to live in `apps/desktop/src-tauri/src/engine_trust.rs`, in the renderer-hosting
//! binary crate, because its one consumer (`ai::governed_sidecar_call`) did. The broker service
//! (§0 role #2) is a separate, synchronous binary that cannot depend on that crate at all, so a
//! second spawn there would have meant a second trust application — one path consulting the
//! provisioned registry and the other the stale committed one, with nothing to say so. The rule
//! is therefore HERE, in the crate both binaries share, and
//! `apps/desktop/src-tauri/src/engine_trust.rs` is now a re-export of it. Both files are in
//! `ai::BRO_PROTECTED_PATHS`.
//!
...
///
/// Takes the SET rather than `brops_provision::Provisioned`, because this crate must not
/// depend on the crate that holds private key halves — `brops_provision::Provisioned::engine_env`
/// is still the single source of the set, and `brops_lib::engine_trust::record` is the one
/// adapter that reads it.
pub fn record(pairs: Vec<(&'static str, String)>, operator_public_key: &str) {
    let _ = RECORDED
        .set(Recorded { pairs, operator_public_key: operator_public_key.to_string() });
}
...
pub struct TrustEnvironment {
    pairs: Vec<(&'static str, String)>,
}

impl TrustEnvironment {
    /// The name/value pairs to set on the child, whole and in the order provisioning produced
    /// them. Read-only: there is no way to add to, remove from, or empty this set.
    pub fn pairs(&self) -> &[(&'static str, String)] {
        &self.pairs
    }
}

/// Resolve the provisioned trust environment for a child that is about to run the engine.
///
...
/// is only as good as the line that calls it, and deleting that line compiled.
pub fn apply() -> Result<TrustEnvironment, String> {
    let recorded = RECORDED.get().ok_or_else(|| NOT_PROVISIONED.to_string())?;
    Ok(TrustEnvironment {
        pairs: resolve(&recorded.pairs, &recorded.operator_public_key, &|n| ambient_env(n))?,
    })
}

/// A `TrustEnvironment` built directly from a set, for tests in THIS CRATE only.
///
/// It exists because `apply` reads a process-global `OnceLock` that other tests in the same
/// binary must be able to find EMPTY — the fail-closed default is itself under test. It is
/// `#[cfg(test)]`, so no production caller can reach it and the compile-time guarantee that a
/// spawn carries resolved trust material is unaffected in a shipping build.
#[cfg(test)]
pub fn test_trust_environment(pairs: Vec<(&'static str, String)>) -> TrustEnvironment {
    TrustEnvironment { pairs }
}
