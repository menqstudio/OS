...
|---|-----|---------|-------------|
| P0-2 | P0 | **Pipe squat + broker impersonation.** Server pipes: NULL DACL + no `FIRST_PIPE_INSTANCE` (squattable); client omits `SECURITY_IDENTIFICATION` (default SecurityImpersonation) → a rogue local server relays a broker-SID token onward and passes the peer-SID gate. | **HALF-FIXED in `f28b559`; the squat half was INERT — corrected 2026-08-07 (remediation R3).** The impersonation half was real (client passes `SECURITY_SQOS_PRESENT \| SECURITY_IDENTIFICATION`). The squat half was not: `FILE_FLAG_FIRST_PIPE_INSTANCE` only fails a create when an instance *already exists*, and the DACL stayed **NULL** — which grants everyone `FILE_CREATE_PIPE_INSTANCE`, so any local principal could add a second instance *after* the server started and collect the next connection. The flag stopped a squatter who arrived first and did nothing about one arriving later, which under a NULL DACL was anyone. Now genuinely closed: an explicit DACL (`win-live/src/pipe_acl.rs` — server principal + SYSTEM + Administrators full, broker SID read/write **without** create-instance, everyone else absent), the client requests an explicit access mask instead of `GENERIC_WRITE` (whose named-pipe generic mapping re-adds that bit), the server holds the pipe name continuously across instances, and a descriptor it cannot build aborts the process rather than falling back. Guarded by unit tests plus a Windows test that reads the DACL back off the live kernel object. **Residual, stated:** in a SAME-ACCOUNT deployment the broker *is* the server principal and keeps create-instance; only a cross-account deployment gets the property, and `run_server` warns when it does not. Note also the original disposition's evidence ("Owner re-ran the live-turn proof"): `win_live_proof.ps1` could not run at all at that commit — see [`CROSS_ACCOUNT_PROOF.md`](CROSS_ACCOUNT_PROOF.md). |
