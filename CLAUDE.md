...
| **2 — Governance Sidecar** | cockpit surfaces for approvals · decisions · evidence chain · signals (mirror, never decide) | 🔨 **8 of 11 boxes** — verified against the code first (T-019); the Owner delegated both remaining decisions on 2026-08-15. §D's `sigbreathe` pulse is **applied and bound to state** (the page was already breathing unconditionally in `blocked`, which is what the not-applied argument existed to prevent). The three open boxes are **one fact**: the approval-**request** path exists on neither side — `T-021`, sequenced behind the standing audit. Evidence sits beside every box in the roadmap |
| **3 — Desktop Integration** | app shell + `home`/`chat`(governed)/`settings`; wire the core loop | ✅ **DONE 2026-08-15 — 11/11**, verified against the code first. 23 routes on a total `Record<RouteId, …>`, error boundary, route-change focus, and a `cmd-dock` that is now a real ARIA dialog. Verification found two live defects: an undeclared `--s7` that silently removed the padding from two empty states, and a modal the keyboard could walk out of |
| **4 — UI/UX System** | component library + theming + motion + a11y; `activity`/`analytics`/`library` | ✅ **DONE 2026-08-16 — 12/12**, verified against the code first. 28 primitives with usage docs, light/dark parity on all 42 colour tokens, four gates in CI (token drift · WCAG-AA on 24 pairs · i18n parity on 233 keys · §C.1). Four §D sweeps found six user-facing defects, including a `command` page that rendered a governed refusal identically to a dropped connection |
| **5 — Memory & Knowledge** | `memory`/`knowledge`/`research`(governed)/`files` | ✅ **DONE 2026-08-16 — 11/11**, verified against the code first. `research` had **no governed run at all** — a local CRUD list in the one page meant to cross the wall — and the files guard was implemented and **never tested** against a merge gate reading *files guard proven*. Both closed. Local writes stay **recorded, not verified**: every write appends a record in the same transaction (migration `0021`); nothing is signed |
| **6 — Multi-Agent** | `agents`/`command`/`tasks`/`projects`; governed pack dispatch | ✅ **DONE 2026-08-16 — 10/10**, verified against the code first. The pages and the dispatch service existed; the gap was the phase’s own stop condition left unasserted — *no desktop-held lease*. Six contract cases now, whitelist not blacklist, and rewriting the builder to spread the assignment turns two of them red. The capability model is unchanged: three tiers, engine-side authority |
...
- **⚠ The canonical-law wall loads from the SESSION's project root, not from the repository you edit.** Found 2026-08-15 (T-019), by the session it had been silently off for. `.claude/settings.json` here wires five events — `SessionStart` / `SubagentStart` / `UserPromptSubmit` / `PreToolUse` / `Stop` — and every one of them is addressed `$CLAUDE_PROJECT_DIR/.claude/hooks/…`. **A session opened at a different directory that then works inside `OS/` gets none of them.** T-019 ran that way (project root `Desktop\ԸՆԳԵՐ`, work in `Desktop\OS`, `CLAUDE_PROJECT_DIR` empty), so for the whole task there was **no full-read receipt, no roadmap-phase declaration, no prior-art check on a new file, and no `Stop` coordination guard** — the `check_read_receipt.py --verify` it should have had answers `RED: no full-read receipt for this session`. This is the same shape as the `engine/` bullet below, one level up: hooks load from the root the session was opened at, and nothing announces their absence. **The CI backstop held and is the reason this is a gap rather than an incident** — `check_canonical_sync` at commit plus the 34 checks ran over whatever landed, exactly as the gate's own "SHELL IS NOT GATED" limit says they must. Two practical consequences: **open the session at `Desktop\OS`** so the wall is live, and treat a session that cannot produce a receipt as one that **may not create a new canonical file** — `--record` would forge it, and the gate's own text names that hole rather than hiding it.
...
- **Session-scoped gates cannot see your session from a bare shell.** `check_read_receipt.py` and `check_roadmap_order.py` resolve the session from `CLAUDE_SESSION_ID`, which the **hooks** set and the Bash/PowerShell tool does **not**. Run them from a tool call without it and you get `RED: no receipt` and a `--session` usage error — neither of which means the gate failed; it means it could not find the session. Export it (`$env:CLAUDE_SESSION_ID = "<id>"`) or pass `--session`. Recorded because a false RED from a gate is indistinguishable from a real one until you know this, and reading one as real is how a session starts chasing a defect that is not there.
...
| **2 — Governance Sidecar** | cockpit surface-եր՝ approvals · decisions · evidence chain · signals (mirror, ոչ decide) | 🔨 **11-ից 8 box** — նախ ստուգված կոդի դեմ (T-019); 2026-08-15-ին Owner-ը երկու մնացած որոշումն էլ պատվիրակեց։ §D-ի `sigbreathe` զարկը **կիրառված ա ու կապած վիճակին** (էջն արդեն անվերապահ շնչում էր `blocked`-ի մեջ — հենց այն, ինչ «չկիրառելու» փաստարկը կանխելու համար կար)։ Բաց մնացած երեքը **մեկ փաստ** են՝ approval-**request** ուղին ոչ մի կողմում չկա — `T-021`, հերթագրված աուդիտից հետո։ Ամեն box-ի կողքին վկայությունը roadmap-ում ա |
| **3 — Desktop Integration** | app shell + `home`/`chat`(governed)/`settings`; core loop-ը wire | ✅ **ԱՎԱՐՏՎԱԾ 2026-08-15 — 11/11**, նախ ստուգված կոդի դեմ։ 23 route՝ total `Record<RouteId, …>`-ի վրա, error boundary, focus, ու `cmd-dock`, որ հիմա իսկական ARIA dialog ա։ Ստուգումը գտավ երկու կենդանի դեֆեկտ՝ չհայտարարված `--s7`, որ լուռ ջնջում էր երկու empty state-ի padding-ը, ու modal, որից ստեղնաշարը դուրս էր գնում |
| **4 — UI/UX System** | component library + theming + motion + a11y; `activity`/`analytics`/`library` | ✅ **ԱՎԱՐՏՎԱԾ 2026-08-16 — 12/12**, նախ ստուգված կոդի դեմ։ 28 primitive՝ usage doc-երով, light/dark parity բոլոր 42 գույնի token-ի վրա, չորս gate CI-ում (token drift · WCAG-AA 24 զույգ · i18n parity 233 key · §C.1)։ Չորս §D սվիպ գտավ վեց օգտատիրոջ երևացող դեֆեկտ, ներառյալ `command` էջը, որ կառավարվող մերժումը ցույց էր տալիս ընկած կապի պես |
| **5 — Memory & Knowledge** | `memory`/`knowledge`/`research`(governed)/`files` | ✅ **ԱՎԱՐՏՎԱԾ 2026-08-16 — 11/11**, նախ ստուգված կոդի դեմ։ `research`-ը **ընդհանրապես կառավարվող կատարում չուներ** — լոկալ CRUD ցուցակ էր այն միակ էջում, որ պիտի պատով անցնի — ու ֆայլերի պահակը կառուցված էր ու **երբեք չփորձարկված**, merge gate-ի դեմ, որ գրում ա *files guard proven*։ Երկուսն էլ փակված։ Լոկալ գրառումները մնում են **գրանցվող, ոչ verified**՝ ամեն գրառում նույն transaction-ում record ա ավելացնում (migration `0021`); ոչինչ ստորագրված չի |
| **6 — Multi-Agent** | `agents`/`command`/`tasks`/`projects`; governed pack dispatch | ✅ **ԱՎԱՐՏՎԱԾ 2026-08-16 — 10/10**, նախ ստուգված կոդի դեմ։ Էջերն ու dispatch ծառայությունը կային; բացը ֆազայի սեփական stop condition-ն էր, որ չէր պնդված — *desktop-ը lease չի պահում*։ Վեց պայմանագրի դեպք, whitelist ոչ blacklist |
| **7 — Group Chat** | `group` համագործակցության սրահ; per-agent governed turn-եր | ✅ **ԱՎԱՐՏՎԱԾ 2026-08-16 — 8/8**, նախ ստուգված կոդի դեմ։ Սրահը, կառավարվող շրջերը, handoff-ի հետքն ու consensus-ի մոդուլը կային; պակասում էր §D-ի room readout-ը։ Լուռությունը դեռ համաձայնություն չի |
...
- **⚠ Canonical-law wall-ը բեռնվում ա SESSION-ի project root-ից, ոչ թե էն repo-ից, որ խմբագրում ես։** Գտնված 2026-08-15 (T-019) — հենց էն session-ի կողմից, որի համար լուռ անջատ էր։ `.claude/settings.json`-ը միացնում ա հինգ event, ու բոլորն էլ հասցեագրված են `$CLAUDE_PROJECT_DIR/.claude/hooks/…`։ **Ուրիշ directory-ից բացված session, որ հետո աշխատում ա `OS/`-ի ներսում, դրանցից ոչ մեկը չի ստանում։** T-019-ը հենց այդպես վազեց (project root՝ `Desktop\ԸՆԳԵՐ`, աշխատանքը՝ `Desktop\OS`, `CLAUDE_PROJECT_DIR`-ը դատարկ), ուրեմն ամբողջ task-ի ընթացքում **full-read receipt չկար, roadmap-phase declaration չկար, նոր ֆայլի prior-art ստուգում չկար, ու `Stop` coordination guard-ը չվազեց**։ Նույն ձևն ա, ինչ ներքևի `engine/` կետը՝ մեկ մակարդակ վերև, ու ոչինչ չի ազդարարում իրենց բացակայությունը։ **CI backstop-ը բռնեց, ու դրա համար ա սա բաց, ոչ թե միջադեպ** — `check_canonical_sync`-ը commit-ի ժամանակ ու 34 check-ը վազեցին։ Երկու գործնական հետևանք՝ **բացիր session-ը `Desktop\OS`-ից**, ու session, որ receipt չի կարա ներկայացնի, **իրավունք չունի նոր canonical ֆայլ ստեղծելու** — `--record`-ը դա կկեղծեր, ու gate-ի սեփական տեքստը այդ անցքն ինքն ա անվանում։
