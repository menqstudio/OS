...
| **2 — Governance Sidecar** | cockpit surfaces for approvals · decisions · evidence chain · signals (mirror, never decide) | 🔨 **Checked against the code, 6 of 11 boxes** (T-019) — the four pages and the read IPC were already built, so the phase was verified before anything else was. The five open boxes are **two facts**: the approval-**request** path exists on neither side (an audited engine task this phase itself defers), and `security`'s §D `sigbreathe` pulse is deliberately not applied because it would paint liveness on a blocked posture. Evidence sits beside every box in the roadmap |
...
| **2 — Governance Sidecar** | cockpit surface-եր՝ approvals · decisions · evidence chain · signals (mirror, ոչ decide) | 🔨 **Ստուգված ա կոդի դեմ՝ 11-ից 6 box** (T-019) — չորս էջն ու read IPC-ն արդեն կառուցված էին, ուրեմն phase-ը նախ ստուգվեց։ Բաց մնացած հինգը **երկու փաստ** են՝ approval-**request** ուղին ոչ մի կողմում չկա (audited engine task, որ այս phase-ը ինքն ա հետաձգում), ու `security`-ի §D `sigbreathe` զարկը միտումնավոր չի կիրառվում, որովհետև blocked posture-ի վրա կենդանություն կնկարեր։ Ամեն box-ի կողքին վկայությունը roadmap-ում ա |
