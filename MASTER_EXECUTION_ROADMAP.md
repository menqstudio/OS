...
| 2 | Governance Sidecar | ✅ **Done 2026-09-19 — 11/11.** The last three boxes were the approval-**request** path (`T-021a`…`T-021d`): a contract audited before it landed, an engine that records an ask and cannot decide it, a desktop command that cannot be told it decided, and a page control named apart from this app's own approval authority. Everything here is ◑ — the Builder's own claim — until someone who did not write it looks. **Reachable at last.** The engine serves a three-valued `brops.governance-read.v1`, the sidecar dispatches named ops, and the desktop no longer requires the AI provider to be `governed-engine` to read a mirror. The mirror was never empty — it was never asked. |
