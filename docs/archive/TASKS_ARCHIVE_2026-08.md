# Task board archive — to 2026-08-29

The coordination board as it stood before `T-045` cut it down: 2758 lines, 42 task rows,
and a per-task essay under most of them. **92% of it was also in `NEXT_CHAT.md`** — the
three coordination documents were one append-log living in three files.

Closed rows and their full narrative live here. `TASKS.md` now carries the OPEN rows and
nothing else. Where this file disagrees with the code, the code wins.
...
### `T-044` — the reverse of the token gate: declared and read by nothing (2026-08-29)

| # | what | status | owner |
|---|---|---|---|
| **T-044** | **`check_c1_tokens.py` asserts every `var(--x)` RESOLVES; nothing asserted the reverse, and the reverse is where residue collects.** `T-033` deleted 1 185 unreachable CSS rules and the custom properties they read survived, because nothing counts a declaration nobody reads: **20 of 149** were dead. The naive scan said 24 — four were BEM modifiers (`.btn--primary:hover`), and the lookbehind that fixes it is the whole correctness of the gate. **Six deleted** as `T-033` residue (`--glow-cyan`, `--hex`, `--hex-clip`, `--mtone`, `--vt`, `--vt-rgb`; `--mtone-rgb` survives beside `--mtone` because something still reads it). **Nine are pinned by §C.1** — including `--hi`, which answers `T-042`'s open note: not a mistake, a specified token with no consumer, and the exemption is read FROM the roadmap so it cannot drift. **Five are allowed by name with a reason**, never by prefix — a property no test asserted until a mutant proved it, and one that now has a test. The allowlist is checked from both sides: an entry naming a token that does not exist, or one something now reads, is RED. | 🔨 Claude | Done — **2026-08-29**; 14 gate tests, wired into CI; mutants: drop the BEM lookbehind ⇒ red, exempt by prefix ⇒ red, stop checking the allowlist ⇒ red | — |

### `T-043` — the second palette gets a committed contract (2026-08-29)

| # | what | status | owner |
|---|---|---|---|
| **T-043** | **`T-042` found six sub-AA values in `aios.css`'s palette by RUNNING a browser; a running test is not a committed contract.** `contrast-pairs.json` carries both palettes now — 14 new pairs, **92 checks** (was 64): `--ink`/`--ink-muted`/`--cyan-soft` on `bg`/`surface`/`raised`, and each of `--cyan`/`--success`/`--warning`/`--mint`/`--danger` on **its own pill tint** at the alpha `aios.css` paints. The mirror assertion routes by family, so editing a colour in one file and not the other is RED in either direction, naming which file. **`--hi` is declared in both `:root` blocks and painted nowhere** — the first draft included it and produced a real 4.32:1 about a background no page has; it is excluded, with the reason written where the list is. And the tint exemption was narrowed: `aios-*-tint` are composites with nothing to mirror, `--menq-color-<name>-tint` are real tokens and stay checked — a blanket rule would have handed back the guarantee `T-042` had just built. | 🔨 Claude | Done — **2026-08-29**; 25 gate tests; mutants: routing, blanket exemption, neutered mirror, and a real-tree `aios.css` edit all red | — |

### `T-042` — axe in a real browser, and the light theme had never been swept (2026-08-29)

| # | what | status | owner |
|---|---|---|---|
| **T-042** | **Phase 10's a11y row waited on one word — `production` — and answering it found eleven defects, ten of them in the light theme.** axe now runs in real Chromium with the stylesheet graph loaded and `color-contrast` ENABLED, over 23 pages × 2 states × **2 themes** plus the shell and the ⌘K palette (98 checks). Three things had to be right first: `@types/jest-axe` had hoisted **axe-core 3.5.6** so a bare import got a 2020 engine that called the ARIA 1.2 combobox a critical violation (pinned to 4.10.2 — the finding vanished, and would otherwise have been "fixed" by breaking correct markup); the light-theme axis was **vacuous** because `AppProvider` overwrites `data-theme` from an effect, and it was green twice before a guard caught it; and entrance animations had to settle or axe measures an animation frame. Fixed: `opacity` on text at 3.07:1 and 4.05:1 (a multiplier is not a colour, so no token pair could see it); two critical ARIA errors (`aria-pressed` on `listitem` → listbox/option; `aria-readonly` on a role-less div); a scrollable region with no tab stop; **six light values in `aios.css`'s own palette**, the one no gate measures, from as low as **1.99:1** on `.eyebrow`; and **all five light `--menq-*` semantics plus dark `danger`**, which sat at 3.65–3.97 on their own badge tint — a background that was in no declared pair. Badges are opaque now (`--menq-color-<name>-tint`), because a translucent one inherits whatever is behind it and measured 4.54 on a surface against 4.26 in a notification row. And `contrast-pairs.json`, a hand-maintained copy of the palette that decides an accessibility verdict, is now **asserted** to mirror `tokens.ts` — it never was. 56 → 64 static checks. | 🔨 Claude | Done — **2026-08-29**; 98 browser checks, 12 measurer tests, 4 mirror tests; mutants: measurer ×4 red, mirror ×2 red | — |

### `T-041` — the performance gate measured one chunk and called it 22 pages (2026-08-29)

| # | what | status | owner |
|---|---|---|---|
| **T-041** | **Phase 10's a11y + performance box, measured before anything was built.** a11y covers all 23 route components and is green; placeholder copy is zero; the Armenian copy is real (238 keys, 2 identical to English, both proper nouns; 1 170 en/hy pairs, 7 identical, all identifiers). **The performance half covered the entry and nothing else** — 23 lazily-loaded chunks, 256.7 KB gzip, no ceiling. `perf-budget.json` gains a `routes` section and `check_bundle_budget.py` enforces it bidirectionally: an unbudgeted route, a stale budget and an over-budget route are each RED, and deleting the section is RED. A route's cost is its closure MINUS the entry's, which is what makes `Chat` 17.71 KB rather than 0.16 KB. **Both roadmap rows stay unticked for one word — `production`: axe runs in jsdom, not against the built app.** | 🔨 Claude | Done — **2026-08-29**, 9 new tests; the route check mutated away turns 4 of them red | — |

### The `contracts/` boxes stay unticked (2026-08-29)

`I-13`'s substance is delivered, but the Phase-10 rows say *duplicates deleted* and two copies of five
files still exist — drift is impossible, deletion has not happened. Both rows are annotated with what is
true and **left unchecked**; the second also names an `approval` schema that exists nowhere in the tree
(`T-021`).

### Two supply-chain gates caught real things on the way in (2026-08-29)

Neither is a task; both are recorded because a gate that fires and is then quietly worked around is
worse than one that never fired.

* **`gitleaks` → `generic-api-key` on `agentsDispatch.boundary.test.ts`.** Correct: the `I-01`
  demonstration restated the 64-hex probe as `const secret = '...'`, a second copy that reads exactly
  like a credential assignment. Fixed by using the probe already declared in `CREDENTIAL_PROBES`.
  **No allowlist entry, no scanner config change.**
* **`cargo-audit` → `chacha20 0.10.1` yanked upstream.** Not this branch's doing; the gate working on
  a new upstream event. Bumped to `0.10.2` (two lines of `Cargo.lock`; `cargo check` clean).
  **Not added to the ignore list**, which is where a yanked crate goes to be forgotten.

### A third load-only flake, recorded and not patched (`T-040`, 2026-08-29)

Found while verifying the ninth-audit remediation, and checked against the pre-change head before being written
down — which is the only reason it can be said not to be that work's fault.

| # | what | status | owner |
|---|---|---|---|
| **T-040** | ⚠ **A THIRD load-only flake, and it is not `T-038`'s cause.** `GroupChat.readout.test.tsx` — *reports the participants and messages it actually read* — fails in a full `npm test` and passes in isolation. **Measured 2026-08-29, five full runs on Windows: three red, two green; the same file run alone is 9/9 every time.** It also fails at `5abdb5e`, *before* the ninth-audit remediation branch existed, so it is **not** caused by that work — the check was run precisely because a new test file changes scheduling and could have exposed it. `Calendar.runs.test.tsx` fell in one of the same runs and not the others. &nbsp; **What the failure says, which is more than `T-038`'s did:** the assertion is `await waitFor(() => expect(valueFor(/PARTICIPANT/i)).toBe('3'))` and the received value is **`'0'`**, not `'(label not found)'`. So the readout element rendered and the roster read had not landed inside the default 1 000 ms — a *timeout on a value that does arrive*, which is a different animal from `T-038` (a language leaking between files; `localStorage` is cleared before every test now and that cause is gone). The full run reports ~1 450 s of environment time across 80 files, so 1 000 ms under that load is not obviously generous. &nbsp; **NOT patched here, deliberately.** Raising the timeout would very likely go green and would also hide a read that never lands — the exact trade `T-038` exists to prevent, and this pull request is audit remediation, not a place to guess. **The decisive experiment, in order:** (1) time how long the value takes to reach `'3'` in isolation, so there is a number to compare against; (2) confirm the same failure under CI on Linux or record that it is Windows-only; (3) only then choose between a longer wait, a deterministic mock resolution, and a real fix in the component's read ordering. Do **not** add `--retry`. | 🔨 Claude | Done — **2026-08-29.** The measurement the row demanded: the value arrives **200 ms** after mount in isolation, against Testing Library's **1 000 ms** `asyncUtilTimeout` — and `vitest.config.ts` had already raised `testTimeout` to 30 s for exactly this reason, so **the suite raised the timeout it knew about and left the one that fires**. The run that took the measurement failed `Approvals.test.tsx` at 1 203 ms instead, `findByRole('dialog')` timing out — different test, same cause, which is what makes it a class. Fixed with neither remedy the row warned about: `asyncUtilTimeout` is 5 000 ms, which hides nothing a broken test would have shown. The risk it does carry is pinned — the isolated latency is asserted at 1 000 ms separately, and a second test proves the ceiling is in force by waiting for a value that appears at 1 500 ms. **3 consecutive full runs 757/757**, against 3 failures in 5 before; mutants: revert to 1 000 ⇒ red, delete the `configure` ⇒ red | — |

### The ninth audit's thirteen findings (`I-01`..`I-13`) — remediation (2026-08-29)

The ninth independent audit filed thirteen findings and **none of them had a row here** until this
change; twelve were still 🔴 OPEN in the ledger five days after they were written, and `I-04` was
marked OPEN two days after its fix had merged. A finding with no owner is a finding nobody is
carrying. Every row below is the **Builder's own claim** and stays ◑ until an independent round
attacks it — that rule is why `A-09` and `T-034` were reopened twice.

| # | P | Finding | Status |
|---|---|---|---|
| **`I-01`** | P2 | **A-09's register calls a credential carrier "shape-constrained."** Eleven leaves bound by `isContractId`/`isRepoPath`/`isWorkPath` admit a credential whole; `slug()` lowercases, so a 64-hex secret in `taskId` arrives verbatim in `contract_draft.task_id`. The file's *"these — and only these"* was false. | 🔨 Claude — **Done 2026-08-29** — the register is COMPUTED from the real validators (19 leaves, not 8) and the audit's attack is a test. Mutant: tighten `task_id` ⇒ 2 red |
| **`I-02`** | P2 | **Three of the eight register entries were never exercised, and deleting them was green.** The suite asserted every *present* leaf is declared and never the inverse. | 🔨 Claude — **Done 2026-08-29** — the real count was **seven**; a `FULL` fixture reaches all of them and `both fixtures really do dispatch` pins the cause. Mutants: 2 red / 2 red / 3 red |
| **`I-03`** | P3 | **One out-of-range byte escaped the decode.** Appending `0x0a` made `decodeCharCodes` return `null` for the whole array. | 🔨 Claude — **Done 2026-08-29** — printable runs + separator-stripped concatenation, in all three copies of the sweep. Mutant in each ⇒ red |
| **`I-04`** | P2 | **`check_contrast.py` decided an accessibility verdict on a rounded number**, and two shipped light pairs sat below AA. | 🔨 Claude — **Done 2026-08-27 (PR #165)** — raw-ratio comparison + re-solved palette; 56 pairs GREEN. *The AUDIT_LEDGER row was left 🔴 OPEN by that PR and is corrected here* |
| **`I-05`** | P3 | **`unittest.main()` sat four lines above `class RestSecondRoad`**, so the file's own entry point ran 74 of 88 and printed `OK`. | 🔨 Claude — **Done 2026-08-29** — entry point last, `FileEntryPoint` keeps it there, 91 from both loaders. Mutant reproduces the audit's 74/`OK` and reds the CI form |
| **`I-06`** | P2 | **The machine mirror's prose drifted** — `purpose` named `b3010f6` and `live_main_resolution` named PR #82. | 🔨 Claude — **Done 2026-08-29** — both restate the live head and the open carrier |
| **`I-07`** | P2 | **The roadmap board disagreed with its own checkboxes on Phases 8 and 9** (8/10 and 8/9 against 7/9 and 7/9), and the gate compared a boolean and never read the fractions. | 🔨 Claude — **Done 2026-08-29** — rows corrected, totals re-measured **92/115** and **44/56**, and `fraction_problems` now counts the boxes. Mutant ⇒ RED naming the phase |
| **`I-08`** | P3 | **`required-checks.json` named the wrong PRs for `T-023`** — #148 against `ci.yml`'s and `TASKS.md`'s #155/#157. | 🔨 Claude — **Done 2026-08-29** — the four are #125/#132/#155/#157, and the row says why the wrong list pointed away from the evidence |
| **`I-09`** | P3 | **The ledger routed the eighth round's §E browser-suite finding to `T-039`.** It is `T-036`. | 🔨 Claude — **Done 2026-08-29** — re-routed, with the misroute recorded rather than erased |
| **`I-10`** | P3 | **Gate counts stale for the second consecutive round** — 19/18 in `START_HERE.md`, 18 in `ARCHITECTURE.md`, **15** in the Armenian half of `START_HERE.md`. | 🔨 Claude — **Done 2026-08-29** — re-measured **23 files / 22 wired**, corrected in all four places, with the measuring command written into the paragraph |
| **`I-11`** | P3 | **Vocabulary enforcement stopped where the entity type says `string`** — `Decision.status`, no CHECK in `0002_decisions.sql`, fixture `'accepted'` through no accessor. | 🔨 Claude — **Done 2026-08-29** — `DECISION_STATUS_FAMILY` + `Decisions.status.ts` + `decisionState()`. `Decision.status` deliberately NOT narrowed and no CHECK added — the ledger is not this app's — and the test says so. Mutant ⇒ TS2345 |
| **`I-12`** | P3 | **`check_bundle_budget.py` had no freshness check** — GREEN at 151.6 KB against a `dist/` built before the deletion it was cited to prove. | 🔨 Claude — **Done 2026-08-29** — a bundled source newer than the manifest is RED naming the file, and no size verdict is printed beside it. Mutant ⇒ 3 red |
| **`I-13`** | P2 | **Two Phase-10 boxes are closable by a Builder change** against the claim that every open box is gate-blocked: `contracts/` was a lone README while `engine/schemas/` held 21 schemas. | 🔨 Claude — **◑ Done except the relocation, 2026-08-29** — `contracts/` is the source, `index.json` versions each contract, and `check_contracts_single_source.py` (17 tests, in CI) fails on drift, on an unclassified schema, on a one-sided version bump and on a stray copy. The move itself needs an audited engine branch — the engine's loaders are root-relative and `engine/` is a subtree — and **that is not the production gate** |
...
| **T-048** | The desktop's governed pre-flight was an assertion, not a measurement: `governed_verification_unconfigured()` returned `Some(...)` hardcoded, so Phase 1's open box *one governed round-trip* sat behind a line no provisioning could move. `governed_provisioning_missing()` reads all five inputs now and the refusal is what it finds; behaviour on the shipped build is byte-identical and the gate stays shut. ◑ | Done | merged `#184` |
| **T-049** | Sixteen roadmap Definition-of-Done rows ended in the middle of a clause: `56e1cd7` ("Make the roadmap fit") got the document under its byte ceiling by deleting the second half of each row. `tools/check_truncated_lines.py` refuses that now, the sixteen are finished from the code, and `test_roadmap_split` enumerates the divergence from the pre-split baseline instead of comparing flat. 22 descriptive bullets in phases 2-6, 8, 9 are cut the same way and stay open. ◑ | Done | merged `#185` |
| **T-050** | 242 declared negative tests, one binding. `SECURITY_NEGATIVE_TEST_MATRIX.md` declares 242 `NM-` ids and exactly one appeared in any source — in a docstring saying the control it names is not implemented. `config/negative-matrix.json` is the generated mirror; `tools/check_negative_matrix.py` refuses drift, an `implemented` whose test does not exist or carry its id, a `blocked` with no `blocked_on`, and any new unreviewed id. 29 implemented, 12 blocked, 201 frozen. The sweep found `_evidence_floor_cas` accepts a higher `head_sequence` on a shorter chain where the matrix requires `evidence_fork` — recorded, not fixed. ◑ | Done | merged `#187` |
| **T-052** | The audit log named a person who was not there: `repo.rs` hardcoded `audit::record(…, "user", "gev", …)` at **34 of 40 call sites** — calls, not lines; five further occurrences are seed fixtures — against the rule stated in that same file, that the actor is passed by trusted code and never hardcoded, so agent events stay distinguishable from human ones (L-4a). An agent repointing a connector was recorded as the human. `audit::Actor` now carries both halves together with no `Default`, so the compiler enumerated every unnamed one, and `tools/check_audit_actor.py` refuses a fresh one **inside the call, not on the line**. Three tier-X commands gained the existing approval gate. `automations::create` no longer writes `enabled = 1`. ◑ | Done | merged `#189` |
| **T-053** | `lstrip("./")` strips a CHARACTER SET, not a prefix. Live in `check_roadmap_order.py` and `check_audit_reports.py`, and the reason a worktree-isolated agent was refused every `Edit` and could only write through the ungated `Bash` path; `check_doc_claims.py` carried the same family from the other side. `check_no_lstrip_prefix.py` refuses the form tree-wide by AST. (2/2) settles the shell AFTER the fact: one new `PostToolUse` event running the same predicates against what changed on disk, because a `PreToolUse` shell path-check is undecidable. It is detection plus halting the turn, NOT containment, and it fired twice on the session that integrated it -- once invited, once not. ◑ | Done | merged `#190`, `#191` |
| **T-055** | **The first produced artifact.** `tools/check_produced_artifact.py` prints GREEN: an agent bundle exists under a digest-named directory whose name is `sha256(manifest.json)`, its flow has two steps, it carries a grant written by `brops_core::agent_bundle::write_grant` rather than by a prompt, `repo::automations::run_due` enqueued a run of it, and the receipt for that run carries `enforcement_regime`. The tick's authority SHRANK: for a bundle it may write a `flow_runs` row and a `scheduler_ticks` row and nothing else. A tampered bundle is REFUSED with a typed reason, never skipped. Three mutation sweeps found three of my own checks tested by nothing and one that was redundant, which split `grant_absent` from `grant_unparseable`. ◑ | Done | merged `#196` |
| **T-051** | Design the production half — the artifact, the flow, the permission grant, the credential path and the scheduled call. Design only, no product code. `docs/design/PRODUCTION_HALF_DESIGN.md` answers what exists at 17:00 that did not exist at 09:00 (files under a digest the Owner confirms); how a customer's request becomes a reviewable flow; the network axis `engine/agents/authority-policy.json` does not have — measured: `USE_NETWORK` is declared in `engine/tools/registry.json` and assigned to `WebSearch`/`WebFetch` with `requires_task`, `requires_scope` and `requires_work_grant` all false, and no lease can carry it because `CLASS_CAPABILITIES` fixes the set; how the customer's own key enters, rests and is used without the agent holding it, as SLOTS inside the bundle with the binding outside it; and how `run_due()` invokes a built agent. §3 and §4 were written by the audit pack and spliced by the main session. ◑ | Done | merged `#194` |
