...
- **`windows-broker-gate` (REQUIRED today).** Runs `python engine/ci/platform_gate_windows.py` and asserts **`platform_governed_execution_supported()` == false** on `windows-latest`. This is the enforced, tested refusal: it fails CI if anyone makes Windows claim support before the real broker lands.
