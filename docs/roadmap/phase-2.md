...
- [x] Read IPC streams engine ledger/evidence/verdicts; approval-**request** path works. — **the read half is complete and wired end to end**: four commands (`governance.rs:614/622/629/637`), registered (`lib.rs`), served by the engine (`bro_control_room_api.py:47`, `:568`, `:616-621`), relayed verbatim (`engine_sidecar.py:477`, `:808`), consumed by the renderer (`desktop.ts:352-366`). **And the request half now exists**, as `T-021` in four steps, each landing on its own: the contract (`contracts/approval-request.schema.json`) **audited before it landed** in [`docs/design/T-021_SCHEMA_AUDIT.md`](../design/T-021_SCHEMA_AUDIT.md), which is the fifth of five invariants `docs/OWNER_ACTION_REQUIRED.md` fixed in advance; the engine's append-only record (`engine/runtime/bro_approval_requests.py`, discharging the five obligations that audit wrote down before the code); the desktop command (`request_engine_approval` over `brops_core::approval_request`, which **blocks** a reply carrying a decision-shaped field whatever else it says); and the page control. Nothing in it decides — only an owner-signed control-room command adjudicates an approval, and this side cannot mint one ◑
...
- [x] `approvals` page (queue + grant/deny/escalate **request**) per §D. — the grant/deny/escalate actions are the **desktop's own** (`T-010`/`T-011`, local ledger, native confirmation), and the **request** across the wall is now a section of its own that says so in its heading, because keeping the two separately named is one of the five invariants. It is usable only when the engine's queue read SUCCEEDED, since an ask needs a `task_id` and the state the engine holds it in, and inventing either would be asking about a task nobody has seen; when the mirror is blocked the section says that instead of offering a disabled mystery. Seven specs in `Approvals.engineRequest.test.tsx`, including that no verdict command is sent and that an outcome claiming a decision is not rendered as one ◑
...
- [x] Contract test: approval-request carries no key/lease; verdicts render faithfully. — the second half held already (`parse_verifier_receipt` enforces `verdict == GREEN`, the id and 64-hex patterns and a non-empty evidence list; `Bridge.test.tsx` drives the real command). The first half was **structurally unwritable** until the path existed — a test asserting that a nonexistent request carries no key is a check that cannot fail — and is written now on both sides of the wall: `brops_core::approval_request` holds the document's key set **by enumeration** and refuses any field named after a key, lease, nonce or verdict, and `approvalRequest.contract.test.ts` holds the renderer to its six inputs. The invariants were fixed before the tests, so the test was not designed by whoever was trying to pass it ◑
