...
- [ ] One governed round-trip proven end-to-end. **Still open, and an independent auditor confirmed why**: the third audit (`AUDIT/2026-08-14-zero-trust-audit-e0dd969.md`) read the three refusals and found all three shut — `governed_verification_unconfigured`, the Linux-only `connect_broker`, and `build_governed_executor`. **That account is true and incomplete, corrected 2026-09-19:** `broker/src/preflight.rs` lists 27 prerequisites for this round trip, and one of them — `custody.committed_label_resolver` — is marked `NotProvisionableOnAMachine` because *"main.rs builds `ChainExecutor::new`, not `with_custody`, so `committed_label()` is None and `persist_committed` REFUSES under EVERY value of the config. No provisioning changes this; it is an owner-gated code decision."* **The decision was taken the same day: wire it** (`T-088`), so the shipped broker now passes `ProductionResolver::custody()` to `ChainExecutor::with_custody` and a turn that verified a manifest under the pinned anchor can commit — labelled for the custody it actually has, since the provenance is derived from which anchor the binary pinned and not from anything a deployment can write. `preflight.rs` marks that row `met-by-build`. What remains for this row is the other **26** prerequisites, none of which is a decision: 13 installer, 11 machine-admin, 1 offline-root-custodian and the Linux platform itself.
