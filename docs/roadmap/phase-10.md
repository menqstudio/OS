...
- [ ] `contracts/` final dedupe (lease/approval/task-contract/mode-grant) + versioning. — **versioning is done, the dedupe is not** (see the Definition-of-Done row above), and `approval` named a schema that existed nowhere in the tree — **it exists now**: `contracts/approval-request.schema.json`, audited before it landed and vendored byte-identical into `engine/schemas/` (`T-021a`). So the row's second blocker is gone and the first is not: the dedupe still needs the engine to read `contracts/` directly, which is an audited engine-subtree change and is why this stays unticked.
