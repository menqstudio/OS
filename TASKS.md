...
>
> **Next:** hand `docs/design/FLOOR_WRITER_SERVICE_DESIGN.md` to the Architect for the §I **step-2 audit**. Its §9 is the checklist: the three-way contradiction over which principal owns the per-install floor, the §2.5/§2.6 amendment an eighth principal requires, the served pin versus a config file both sites read, whether the per-task floor should adopt the governed floor's A–E matrix, and the `_head_floor_dir` warning that goes stale the day FW-1 lands. **No FW slice may start before that verdict**, and FW-3 additionally needs the amendment ratified. `B-02` and `A-01`'s root close when the design is Architect-GREEN **and built** — not when #112 merges. **#112 is deliberately left OPEN for that audit** — the design-PR precedent here is PR #30, which merged only after rev-5 was Architect DESIGN GREEN; CI is 33/33 on the exact head `693bc42` and merging is the Owner's or the Architect's call, not a Builder step.
>
...
> **▶ CURRENT STATE tokens:** `CURRENT_ACTIVE_TASK: T-020` · `CURRENT_ACTIVE_WAVE: floor-writer` · `CURRENT_FLOOR_WRITER_DESIGN: PROPOSAL_PENDING_ARCHITECT_AUDIT` · `CURRENT_PHASE0: done` · `CURRENT_DESIGN_GATE: OWNER_APPROVED_NOT_ARCHITECT_AUDITED` · `CURRENT_DESIGN_CANDIDATE: rev-30` · `CURRENT_LAST_REVIEWED: rev-30` · `CURRENT_LAST_VERDICT: OWNER_APPROVED_NOT_ARCHITECT_AUDITED` · `CURRENT_DESIGN_PR: 48` · `CURRENT_IMPL_PR: 48` · `CURRENT_IMPL_STATE: consolidated` · `CURRENT_CODE_AUDIT: ARCHITECT_PENDING` · `CURRENT_LINUX_E2E: proven` · `CURRENT_WINDOWS_LIVE_PROOF: proven` · `CURRENT_PRODUCTION_VERIFIED: false` · `CURRENT_VERIFY_SEAM: complete` · `CURRENT_RECEIPT_PLUMBING: complete` · `CURRENT_GOVERNED_ROUNDTRIP: complete`
...
| **T-020** | 🛑 **security-audited (before implementation)** — **The floor-writer service design proposal.** `docs/OWNER_ACTION_REQUIRED.md` §1b was **decided by the Owner on 2026-08-14** (option 1: a resident principal owns the marks directory; the setuid helper is not taken) and ships with its own process — *"Owner approval (given, here) → Architect audit → implement. **No implementation lands on this decision alone.**"* Step 2 could not happen because step 1 did not exist: no floor-writer design was in the repository, while `AUDIT_LEDGER.md`'s `B-02` row linked [`docs/design/FLOOR_WRITER_SERVICE_DESIGN.md`](./docs/design/FLOOR_WRITER_SERVICE_DESIGN.md) by path and `bro_completion.py:502` named the service as the resolution. This row is that file: **a proposal, not an implementation.** It specifies one resident principal (`brops-floor` / `NT SERVICE\BroPSFloorWriter`) that owns `bro_completion`'s per-task anti-rollback marks **and** serves the deployment's `install_id` — the Owner's reason 3, *"one principal, one trusted config, both defects closed at the same boundary"* — with the policed account given **no access at all** (the parent-rename verdict at `bro_completion.py:593-600` is why the marks leave `BRO_EVIDENCE_STORE`), the scope pin resolved **once at start** so `B-02`'s objections to both *moving* and *duplicating* the check are answered, a closed two-state posture with **no fallback**, and the limit stated inside the document (moving the write makes the **mark** unforgeable, not the **check** unskippable). Transport is reuse on both platforms: `SO_PEERCRED` per `isolated_signer_server.py`, the named-pipe DACL + kernel-attested SID per `win-live/src/pipe.rs`, one framing and **one** cap number in both directions. It is **also an amendment request** to the rev-30 addendum's normative §2.5 (`TCB_ARTIFACTS`) and §2.6 (**SEVEN** runtime service UIDs becomes eight) — the Architect's to ratify, and FW-3 is blocked on it. &nbsp; **Finding reported, not fixed:** one anti-rollback floor has **three named owners** — addendum §7 P1-7 (`:1142`, `:3355`) and `core/src/supervisor_ledger.rs:20` say `brops-signer`-owned; the DDL (`supervisor_ledger.sql:168`), the CAS (`governed_supervisor_ledger.py:774`, inside the supervisor's transaction) and the only process that opens the file (`run_supervisor.py:155`) say the supervisor, and `isolated_signer.py` opens no sqlite at all; the scope key's pin is the challenge authority's. That is `B-02` in full and it is §9 question 1. **Deliberately NOT done:** relocating `_evidence_floor_cas` (it is folded into one `BEGIN IMMEDIATE` so *"a refused floor cannot leave a completion behind"*), changing the floor rule in the same slice as the write, and closing `O-2`/`O-5`/`F-29`. Every claim about current code is **◑ — the Builder's own reading with `file:line` beside it**; nobody independent has read it. | 🔨 Claude | **Review** — proposal authored, awaiting the 📐 Architect §I step-2 audit. **No FW slice may start before that verdict.** `B-02` closes when the design is Architect-GREEN **and built**, not when this merges. | `design/floor-writer-service` → **PR #112** |
