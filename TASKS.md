...
> **⏭️ CURRENT ACTIVE: PR #122 · branch `phase4-analytics-scrubber`** (base `main`, tip `3da4cd1`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Phase 4: the analytics page had no scrubber at all, and section D names one. Built as a full ARIA slider over the RANK cut-off rather than a timeline, because get_analytics returns one all-time aggregate with no time dimension and drawing an axis the engine does not expose is what this page already refuses to do in three other panels.
>
