...
|----|------|-----------|--------|-------------|
| **T-047** | **The tool that stamps `AUDIT_CANDIDATE_HEAD` could not write a pull-request body here.** `tools/stamp_pr_head.py` wrote through `gh pr edit`, which resolves the PR over GraphQL and asks for `repository.pullRequest.projectCards` — sunset with Projects (classic). On gh 2.46.0, the version Debian ships, it dies before writing, so the marker stays stale and `check_repo_state.py` goes red on the next push for a reason unrelated to the push. It writes over REST now and reads the body back. 8 tests, both guarantees mutation-checked. ◑ Builder's claim | 🔨 Claude | Done | `#183` |
