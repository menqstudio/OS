...
> **⏭️ CURRENT ACTIVE: PR #117 · branch `phase3-shell-a11y-and-c1-gate`** (base `main`, tip `5d3b456`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Phase 3 shell work: --s7 and --s9 were missing rungs of the --s1..--s10 ladder while var(--s7) shipped on two empty states (whole declaration dropped, no padding); the command palette gained real dialog/combobox semantics, a focus trap, focus restore and its first tests; and tools/check_c1_tokens.py turns section C.1 into a gate that catches exactly that class of bug.
>
