...
|----|------|-----------|--------|-------------|
| **T-111** | **Nothing in the tree had ever written a `$BROPS_BROKER_CONFIG` document** - the writer for a real deployment, with the broker's own 25-key read set, the §2.5 roster and the four `sidecar` rules mirrored and BOUND to the Rust that defines them; 8 mutants, 8 named deaths ◑ | Bro | In-Progress | `#283` |
