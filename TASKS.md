...
> **⏭️ CURRENT ACTIVE: PR #132 · branch `phase8-calendar-runs`** (base `main`, tip `6ec2674`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Phase 8 records at 8 of 10. Two boxes stay unticked because the code already established why: run_automation is a local write, not a governed dispatch, so its engine_receipt evidence is permanently unobserved and there are no receipt ids to show. Opened T-022 for the governed dispatch. The calendar had no run history at all; it has one now, and it states in one line that no engine receipt exists rather than leaving a column that would read as pending.
>
