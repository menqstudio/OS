...
|----|------|-----------|--------|-------------|
| **T-049** | **Sixteen roadmap Definition-of-Done rows ended in the middle of a clause.** `56e1cd7` ("Make the roadmap fit") got the document under its byte ceiling by deleting the second half of each row — the budget gate can only be satisfied by removing text, and the cheapest text was the end of every sentence. Twelve Phase-1 rows and four Phase-10 rows — the rows `check_roadmap_order.py` reads to decide which phase may be worked — and thirty gates passed over them, because every one of them judges content that is PRESENT. `tools/check_truncated_lines.py` is the counterweight; the sixteen are finished from the code, every claim re-read today; `test_roadmap_split` ENUMERATES the divergence now instead of comparing flat. **Still open:** 22 descriptive bullets in phases 2-6, 8, 9 are cut the same way, measured and not covered ◑ | 🔨 Claude | Review | `fix/truncated-canon` |
