...
| **T-056** | **Two fail-closed checks prevent nothing, now declared rather than hidden.** `config/control-invocation.json` derives 56 controls from the filesystem and holds each to what its failure stops. Open on the two: `bro_deploy_preflight.py` has zero non-test callers and is named 3× in `OPERATOR_RUNBOOK.md` — a suggestion; `check_ai_surfaces.py` runs under a context NOT in the required 33. Making a context required is branch protection, the Owner's act | Bro | Todo | `#208` |
