...
|----|------|-----------|--------|-------------|
| **T-055** | **The first produced artifact — one agent, built by the factory, not by hand.** Its five conditions, and which are missing, are printed by the produced-artifact gate, RED by design until they are met — that output IS the definition of done, so nobody must remember it and nobody can quietly soften it. **Point 3 decides it: the grant is written by the RUNTIME, not a prompt.** Linux only. Do not open the governed gate. If one of the five cannot be met honestly, stop and say which. **blocked_on:** that gate and the production-half design, both written, neither merged | — | Todo | — |
| **T-052** | **The audit log named a person who was not there.** `repo.rs` hardcoded `audit::record(…, "user", "gev", …)` at **34 of 40 call sites** — calls, not lines; five further occurrences are fixtures — against the rule in that same file: the actor is passed by trusted code, *never hardcoded*, so agent events stay distinguishable from human ones (L-4a). An agent repointing a connector recorded as the human. Now an `audit::Actor` with both halves derived and no `Default`, so the compiler enumerated every unnamed one. The new gate refuses a fresh one **inside the call, not on the line**. Three tier-X commands gained the existing approval gate ◑ | 🔨 Claude | Review | `t052/audit-attribution` |
...
| **T-030** | **Route 1 past the no-lease / no-secret whitelist** (`A-09`, reopened by the ninth audit). Routes 2 and 3 are closed and mutation-confirmed. Route 1 is open **by design**: a credential is defined by what a remote system accepts, not by its text. The register is COMPUTED from the real validators — **19 leaves, not 8** — but the route is not closed | — | Todo | — |
