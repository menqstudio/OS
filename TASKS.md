...
|----|------|-----------|--------|-------------|
| **T-108** | **`trusted_verified` was unreachable on Linux by construction; it is not now** - the ceremony is two phases, the kit verifies the signature AND that the manifest names its keys, and no root private touches the serving box ◑ | Bro | Review | `#279` |
