...
| **T-010** | 🛑 **security-audited** — Tauri capability boundary: the SQLite-backed / AI-exec / runs / automations / integrations **mutation** commands are registered to the webview but **not capability-gated** (gating is a `TODO` in code). Define + enforce Tauri capabilities so webview-reachable mutations are scoped to what each surface may do. **Audited security-design task, not a quick fix.** **Wave 2b design-first:** joint T-010+T-011 design (privilege topology, 68-command inventory + risk tiers, deny-by-default manifest, in-command enforcement, negative tests, rollout) in [`docs/design/WAVE_2B_CAPABILITY_APPROVAL_DESIGN.md`](./docs/design/WAVE_2B_CAPABILITY_APPROVAL_DESIGN.md) (Architect **APPROVED** rev 2). **Implemented:** AppManifest → all 65 commands; `capabilities/default.json` deny-by-default (`deny-decide-approval`, `allow-reject-approval`, rest granted per tier); new `reject_approval` fail-safe command (rate-limited, pending-only, atomic) — generic `decide_approval` denied to `main` + approve fails closed until T-011 native confirmation; in-body bounds on `create_automation`; CI invariant `tools/check_capabilities.py` (registered == manifest == policy == grants) + [`command-policy.json`](./apps/desktop/src-tauri/command-policy.json) + [in-body audit](./docs/design/T-010_INBODY_AUDIT.md). Tests: capability gate self-tests + Rust rate-limit/bounds. | 🔨 Claude | Review | `feat/t-010-capability-boundary` |
