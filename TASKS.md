...
> **⏭️ CURRENT ACTIVE: PR #134 · branch `phase10-m1-schema-mirrors`** (base `main`, tip `da75c2b`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Phase 10 is mostly Owner and audit gated; M1 of the contracts dedupe is not, and it needs no file to move. The Rust structs that claim to mirror an engine schema were bound to it by nothing but a doc comment; they are bound by a gate now. The gate was wrong first - it did not know a const property is a discriminator - and fixing that made the rule stronger rather than looser.
>
