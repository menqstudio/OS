...
> **⏭️ CURRENT ACTIVE: PR #118 · branch `phase3-close`** (base `main`, tip `8259a3a`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Phase 3 closes at 11 of 11, verified against the code before anything was built. The contracts dedupe plan is measured: four schema homes not two, no duplicated schema file anywhere, and the real drift is a Python schema bound to a hand-written Rust type by nothing but a doc comment - so the milestone's first step is a binding gate, not a move. It also records that the approval schema named by contracts/README.md and by this phase's own Contracts row does not exist, which is the same absence Phase 2 found from the other end (T-021).
>
