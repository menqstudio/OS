...
| **T-062** | **189 negative-matrix rows are still `unreviewed`** — nobody has looked, which is the silent one. 12 moved on 2026-08-31: 3 to `implemented` (mutation-proven) and 9 ACL rows to `blocked` on a measured cause — the `sup/`+`rec/` store split exists only in two docstrings. The remaining priority domains are TIME 12, ACL 6, REPLAY 7, CONC 5, REG 5, EVID 3 | — | Todo | — |
| **T-056** | **Every fail-closed check must name what its failure PREVENTS** — merge, session, deploy, release or nothing — in a registry, not a docstring. A check whose consequence is `nothing` is RED: a runbook mention is a suggestion. The population is DERIVED from the filesystem, so the gate cannot omit itself. PR #208 | — | Todo | — |
