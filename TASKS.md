...
> **⏭️ CURRENT ACTIVE: PR #128 · branch `phase5-governed-research`** (base `main`, tip `9aa1a52`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Phase 5: the research page was a local CRUD list with no run, no receipt and no refusal - the one page in the phase whose point is that it crosses the wall. It now runs through stream_ask, the same governed path chat uses, with a new save_ask_to_knowledge command that takes the one-time id and never a body. In the shipped app it renders blocked, because the gate is deliberately shut; that is the working run reporting what the wall said.
>
