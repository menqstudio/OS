...
| **T-056** | **Every fail-closed check must name what its failure PREVENTS** — merge, session, deploy, release, or nothing — in a registry, not a docstring. A file whose docstring says *fail-closed* and whose consequence is `nothing` is RED: being named in a runbook is a suggestion. `bro_deploy_preflight` is the worked example. The population is DERIVED from the filesystem, so the gate cannot omit itself. Also fixes three dead `rust_mirror` paths in the contracts index. **Deferred behind `T-055` by the Owner deliberately — it catches FUTURE defects and HAS a finish line while `T-055` does not, and work runs where one is. Reordering needs a written reason** | — | Todo | — |
| **T-053** | **`lstrip("./")` strips a CHARACTER SET, not a prefix** — live in two gates, and the reason a worktree-isolated agent was refused every `Edit` and could only write through the ungated `Bash` path. A tightening in both directions: `.claude/` and `.github/` are visible to the `meta` scope again, and `../tools/x.py` is no longer laundered into an allow-listed prefix. `check_no_lstrip_prefix.py` refuses the form tree-wide by AST. `test_wall_bash_gap.py` records what the root coordination gate does NOT see — the engine's enforcement wall matches `*` and does see Bash; four session-scoped protections do not. The containment half is `T-053b`, held for a quiet tree ◑ | 🔨 Claude | Review | `t053a/lstrip-evidence` |
| **T-046** | **The Windows engine job must run clean across several pull requests** before the ledger's concurrency flake is called fixed — one green run does not prove an intermittent (`T-023`'s lesson). Merged as PR #182; open on the EVIDENCE, not the code | — | Todo | merged `#182` |
