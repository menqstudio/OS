...
> **⏭️ CURRENT ACTIVE: PR #124 · branch `phase4-automations-slash`** (base `main`, tip `c890a1a`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Swept all 22 pages' section-D keyboard contracts against the code and found one real gap: automations declares a / filter binding and had none. Built as a focus move into the state-filter chip group, because this page filters with chips rather than a text field and binding / to a search box that does not exist would mean building a second filter to satisfy a keystroke.
>
