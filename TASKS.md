...
> **⏭️ CURRENT ACTIVE: PR #121 · branch `a11y-pages-coverage`** (base `main`, tip `6af1ccc`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> The axe pass now covers the pages, not only the shared primitives - the fifth audit's section E. Six pages in two states each (with data, and engine-unreachable), plus the app frame and the command dock while open. The reason no page spec could have existed before: a11y-setup.ts ran without the jsdom stubs setup.ts installs, so any page reading matchMedia at mount threw before axe could look at it.
>
