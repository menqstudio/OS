...
> **⏭️ CURRENT ACTIVE: PR #125 · branch `command-blocked-state`** (base `main`, tip `861cc69`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Section-D states sweep across all 22 pages. The command page renders the one action in this cockpit that crosses the governed wall, and a refusal came back looking exactly like a dropped connection - one warn pill, raw string, announced to nobody. A refusal is the wall working; a failure is something broken. They are told apart now, the engine's reason is carried verbatim, and the outcome is announced rather than queued behind the stream.
>
