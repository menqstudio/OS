...
| **T-056** | **Two fail-closed checks prevent nothing, now declared rather than hidden.** `config/control-invocation.json` + `check_control_invocation.py` derive 56 controls from the filesystem and hold each to what its failure stops. Open on the two: `bro_deploy_preflight.py` has zero non-test callers and is named 3× in `OPERATOR_RUNBOOK.md` — a suggestion; `check_ai_surfaces.py` runs on every PR under a context NOT in the required 33. Making a context required is branch protection, the Owner's act | Bro | Todo | `#208` |
| **T-057** | **56 fabricated audit rows are indistinguishable from real ones.** `repo::seed` writes them by raw SQL (`repo.rs:3275-3278`); no `source` column. **Closure: a reviewer reading `audit_events` can tell fabricated from real WITHOUT reading `repo.rs`.** A column nothing surfaces does not close it. `deferred_until: 2026-09-06` | — | Todo | — |
| **T-046** | **The Windows engine job must run clean across several PRs** before the ledger's concurrency flake is called fixed — one green run does not prove an intermittent. Open on the EVIDENCE | — | Todo | merged `#182` |
