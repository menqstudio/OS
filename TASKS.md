...
> **⏭️ CURRENT ACTIVE: PR #126 · branch `a11y-contract-gaps`** (base `main`, tip `4695dbe`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Third section-D sweep - the A11y line this time, after keyboard and states. 21 pages, two real gaps: the tasks board lanes were bare divs rather than lists, so a screen reader read the cards as loose content with no count or position, and the command dispatch trace carried aria-live without role=log. The visually-hidden rule is also promoted from .chart-caption to .sr-only, which is why two other pages had re-declared it locally.
>
