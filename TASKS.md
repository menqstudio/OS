...
> **⏭️ CURRENT ACTIVE: PR #123 · branch `phase4-library-enter`** (base `main`, tip `c32b4e3`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> Phase 4: section D binds Enter to open on the library page. The rows are buttons so Enter already fired an onClick that only re-selected the already-selected row, while the preview panel it should open had no tab stop at all - a keyboard user watched it change and could never reach it. Enter now hands focus into the preview and Escape hands it back to the row it came from.
>
