...
|----|------|-----------|--------|-------------|
| **T-110** | **Nothing in the tree had ever run the `brops-broker` binary** - the first integration test spawns it over a real AF_UNIX socket and pins the fail-closed refusal, the peer check and that one bad peer cannot wedge the loop ◑ | Bro | Review | `#282` |
