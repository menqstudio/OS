...
> **⏭️ CURRENT ACTIVE: PR #120 · branch `settled-head-pin`** (base `main`, tip `b37d80a`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> The A-07 floor shipped in #119 was unsatisfiable for a self-carrier and turned main red on the next merge: the snapshot is written inside the pull request, before the merge it would have to postdate. Replaced with an exact pin - settled_at_main_head is the merge commit's FIRST PARENT, the main the carrier merged into - and the generator now advances it on every sync rather than only on --settled, which is how it drifted three merges behind with nothing able to see it.
>
