...
> **⏭️ CURRENT ACTIVE: PR #136 · branch `t004-o-items-are-yours`** (base `main`, tip `d816f5b`, task T-017). Also open, and not this PR's work: PR #112 on `design/floor-writer-service`.
>
> T-004 worked under the Owner go. Every one of O-1..O-5 has its code half built and a remaining half that is a deployment act only the Owner can perform; none needs an offline-root-signed secret. All five are now stated as decisions in OWNER_ACTION_REQUIRED section 2c with the one act that closes each and the owner-signed-deferral alternative. The finding: that page listed four of the five under 'Open, and not waiting on you'.
>
