...
class DocsFreshnessTests(unittest.TestCase):
    def test_every_document_is_registered_and_reviewed(self):
        self.assertGreaterEqual(validate_docs(ROOT), 62)

    def test_post_pr8_manifest_metadata_is_exact(self):
        data = json.loads(
            (ROOT / "config" / "documentation-manifest.json").read_text(encoding="utf-8")
        )
        validate_manifest_metadata(data)

    def test_pre_pr8_manifest_metadata_is_denied(self):
        data = json.loads(
            (ROOT / "config" / "documentation-manifest.json").read_text(encoding="utf-8")
        )
        data["merged_pr"] = 6
        data["merge_commit"] = "2395570bc9571e6c721373751a6dbfa2b6a8f75b"
        data["status"] = "orchestration-runtime-v1-foundation-merged-control-room-api-v1-next"
