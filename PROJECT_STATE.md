...
**Last updated · Վերջին թարմացум:** 2026-07-27 (Phase-0 repository-truth correction round 5 — structured transition-aware carrier resolution).
...
> `CURRENT_ACTIVE_TASK: T-017` · `CURRENT_ACTIVE_WAVE: 3b-1B` · `CURRENT_DESIGN_GATE: PENDING_REAUDIT` · `CURRENT_DESIGN_PR: 31` · `CURRENT_IMPL_PR: 32` · `CURRENT_IMPL_STATE: WIP` · `CURRENT_REPO_TRUTH_PR: 33` · `CURRENT_PRE_MERGE_GATE: PR33_REAUDIT` · `CURRENT_POST_MERGE_GATE: REBASE_PR31` · `CURRENT_PRODUCTION_VERIFIED: false` · `CURRENT_VERIFY_SEAM: complete` · `CURRENT_RECEIPT_PLUMBING: complete` · `CURRENT_GOVERNED_ROUNDTRIP: complete`
> Structured carrier-resolution tokens (both branches): `CARRIER_IF_OPEN_GATE: PR33_REAUDIT` · `CARRIER_IF_MERGED_GATE: REBASE_PR31` · `CARRIER_IF_OPEN_PHASE0: in_progress` · `CARRIER_IF_MERGED_PHASE0: done`
>
> Wave 3a slices 1–3 are **merged, zero-trust GREEN** — the verify-seam, receipt-plumbing, and the real fail-closed governed round-trip **all landed** (PR #28). Wave 3b-0 design **merged** (PR #30). **PR #31** (`feat/wave-3b1-isolated-signer`) = 3b-1A **Code GREEN** + the rev-26 design **candidate, PENDING re-audit** (not GREEN). **PR #32** (`impl/wave-3b1b-execution-binding`, Draft) = **WIP** 3b-1B implementation (a draft checkpoint, not an RC). **No production `trusted_verified`** (`NoTrustedManifest` fail-closed). **Open:** the remaining AI entry points + **Windows production isolation** + Phases 2–10.
>
> **Carrier resolution — the repository-truth PR #33 (`chore/phase0-repository-truth`); the CURRENT truth branches on its live state (transition-aware, so this cannot go stale on merge):**
> - **IF OPEN** → Phase 0 = `in_progress`, gate = `PR33_REAUDIT`; the next action is repository-truth GREEN on the exact candidate (PR-body `AUDIT_CANDIDATE_HEAD`) and merge PR #33 (do not merge until GREEN).
> - **IF MERGED** → Phase 0 = `done`, gate = `REBASE_PR31`; the next action is verify main CI, rebase PR #31, synchronize state, resolve PR #31's new exact HEAD, remediate rev-26's preparation-P0, then the exact-head Architect design audit on that new HEAD.
>
...
1. **Wave 3b — isolated signer + signed manifest + production "Verified" (T-017)** — fill the
   ...
   progress** — PR #31 (`feat/wave-3b1-isolated-signer`) = 3b-1A code Architect Code GREEN + rev-26 design
   candidate PENDING re-audit; WIP implementation in PR #32 (Draft, not an RC). **Next permitted action —
   resolve the repository-truth PR #33 live; the step branches on its state (transition-aware):**
   **IF OPEN** → gate `PR33_REAUDIT`, Phase 0 `in_progress`; obtain repository-truth GREEN on the exact
   candidate (PR-body `AUDIT_CANDIDATE_HEAD`) and merge PR #33 (no merge until GREEN). **IF MERGED** →
   gate `REBASE_PR31`, Phase 0 `done`; verify new `main`, rebase PR #31 onto it, resolve PR #31's NEW
   exact HEAD (NOT the pre-rebase `6ebeca8`), remediate rev-26's 3 preparation-P0, then submit rev-26 for
   the Architect design audit on that new HEAD. No merges until an exact-head verdict; `NoTrustedManifest`
   stays fail-closed (no production "Verified" yet).
