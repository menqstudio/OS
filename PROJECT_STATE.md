...
> **✅ SETTLED — `main` is at `97c94ec`.** Nothing is open. Start from `docs/OWNER_ACTION_REQUIRED.md`, the one page that says what is blocked and on whom.
>
> **Next:** One of the third audit's five findings remains: A-02, the run-evidence chain's hash link is written and never checked - previous_event_hash is recomputed by no consumer and final_event_hash, the field EvidenceFork compares, is copied off the document with only a format check. The rule to reproduce is in win-live/src/execution.rs:159-176: event_hash = sha256(canonical(event)) chained, final_event_hash = the last one. Both consumers need it: governed_supervisor_ledger.py:640-685 and win-live/src/servers.rs:386-430. Also open, from A-05's own note: no test feeds a Linux-written chain to the Windows parser, and the Windows writer still emits serde_json::to_vec. A-01/A-03/A-04/A-05 are Builder-closed; A-01 and A-05 are circle-half, not confirmed.
>
