...
> **⏭️ CURRENT ACTIVE (2026-08-06): PR #54 · branch `fix/release-setup-node-pin`** (base `main`, task T-017) — a supply-chain hotfix against `main`. `.github/workflows/release.yml` pinned `actions/setup-node` to a SHA that does not exist in that repository, so the release workflow could not resolve its own action; it is re-pinned to the real v4.1.0 commit, and `tools/check_action_pins.py` (11 self-tests, wired as the supply-chain `action-pins` job) now fails the build on any unresolvable or unpinned action reference rather than leaving it to be found at release time. It also carries the two supply-chain remediations `main` still lacks — the `undici` ^7.29.0 override that clears the un-waived high npm advisory, and the three triaged `cryptography` X.509/PKCS#7 waivers for surfaces the engine's Ed25519-only usage never reaches — so this PR is green on its own base instead of waiting on PR #53. PR #53 (`feat/windows-broker-machineproof`) remains the larger keystone workflow. Earlier merged-PR prose below is HISTORY.
...
> `CURRENT_ACTIVE_TASK: T-017` · `CURRENT_ACTIVE_WAVE: 3b-1B` · `CURRENT_PHASE0: done` · `CURRENT_DESIGN_GATE: GREEN` · `CURRENT_DESIGN_CANDIDATE: rev-30` · `CURRENT_LAST_REVIEWED: rev-30` · `CURRENT_LAST_VERDICT: GREEN` · `CURRENT_DESIGN_PR: 48` · `CURRENT_IMPL_PR: 48` · `CURRENT_IMPL_STATE: consolidated` · `CURRENT_CODE_AUDIT: ARCHITECT_PENDING` · `CURRENT_LINUX_E2E: proven` · `CURRENT_PRODUCTION_VERIFIED: false` · `CURRENT_VERIFY_SEAM: complete` · `CURRENT_RECEIPT_PLUMBING: complete` · `CURRENT_GOVERNED_ROUNDTRIP: complete`
>
