...
> `CURRENT_ACTIVE_TASK: T-017` · `CURRENT_ACTIVE_WAVE: 3b-1B` · `CURRENT_PHASE0: done` · `CURRENT_DESIGN_GATE: RED` · `CURRENT_DESIGN_PR: 31` · `CURRENT_IMPL_PR: 32` · `CURRENT_IMPL_STATE: WIP` · `CURRENT_PRODUCTION_VERIFIED: false` · `CURRENT_VERIFY_SEAM: complete` · `CURRENT_RECEIPT_PLUMBING: complete` · `CURRENT_GOVERNED_ROUNDTRIP: complete`
>
> Wave 3a slices 1–3 are **merged, zero-trust GREEN** — the verify-seam, receipt-plumbing, and the real fail-closed governed round-trip **all landed** (PR #28). Wave 3b-0 design **merged** (PR #30). **Phase 0 (repository-truth remediation) is DONE — merged via PR #33** (Owner-approved GREEN at exact `45f3793`, squash **`b6c6712`**; the Phase-0 carrier machinery is retired). **PR #31** (`feat/wave-3b1-isolated-signer`, **rebased onto the repaired `main` `b6c6712`**) = 3b-1A **Code GREEN** + the 3b-1B design addendum which is **Architect design RED** — **rev-26 got 2 P0 + 3 P1** at `b604cbc`; **rev-27** remediates and is re-submitted. PR #31 is the snapshot's **current_workflow_pr**, exact-head-anchored by its PR-body **`AUDIT_CANDIDATE_HEAD`** marker (nothing is exempt from exact-head verification). **PR #32** (`impl/wave-3b1b-execution-binding`, Draft) holds **UNAPPROVED Draft/WIP** 3b-1B code with **no authority over the design** (not an RC, not Architect-approved; adapt only after design-GREEN; frozen from architectural expansion). **No Architect-approved or merged 3b-1B implementation exists. No production `trusted_verified`** (`NoTrustedManifest` fail-closed). **Open:** the 2 P0 + 3 P1 rev-26 blockers, the remaining AI entry points, **Windows production isolation**, Phases 2–10.
>
> **Next action:** the earlier 3 preparation-P0 are closed (§2.5/§2.6/§0.1/§0.W), but the **Architect verdict on rev-26 is RED**. rev-27 closes the two P0 (P0-1 eight-principal challenge-authority topology + desktop-client/authority separation + 3 threat actors; P0-2 Model A root/TCB launcher) — then **re-submit rev-27 for the Architect design audit at PR #31's exact HEAD**. Preparation-review closure is NOT the Architect's verdict; do **not** merge PR #31 until rev-27 is Architect design-GREEN at the exact HEAD.
>
...
1. **Wave 3b — isolated signer + signed manifest + production "Verified" (T-017)** — fill the
   ...
   the 3b-1B design addendum which is **Architect design RED** (rev-26 = 2 P0 + 3 P1 at `b604cbc`); PR #32
   holds UNAPPROVED Draft/WIP code (no design authority). **Next permitted action:** rev-27 closes the two
   ...
   the new exact head, then **re-submit rev-27 for the Architect design audit at PR #31's exact HEAD**. No
