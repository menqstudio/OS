...
> **✅ SETTLED — `main` is at `d0bddc4`.** The pull request that records it is PR #162 on `settle-after-A`. Also open, and deliberately not merged here: PR #112 (`design/floor-writer-service`). Start from `docs/OWNER_ACTION_REQUIRED.md`, the one page that says what is blocked and on whom.
>
> **Next:** Block B is done -- the NINTH independent audit ran on this head and returned **RED**. Six of the nine Builder claims earned the ledger's independently-confirmed mark; `A-09` and `T-034` are **REOPENED**; `T-023` is held at `◑`. The next step is the remediation of `I-01`..`I-13`, and `I-04` comes first because it is a live WCAG AA defect the gate built to catch it reports as GREEN.
>
> **The last independent audit returned RED, and it is the NINTH.** `apps/desktop/AUDIT/2026-08-19-ninth-audit-5cf9b8c.md`, of `main` @ `5cf9b8c` (tree `9580b86d`), pin proved on a clean worktree before the first file was opened -- `rev-parse ^{tree}` == `write-tree` == the pin. **No P0:** all three production-gate refusals were read at the source and are closed for the fourth round running, `AnswerProvenance::Governed` is constructed only inside `#[cfg(test)]`, and nothing in the tree sets `$BROPS_BROKER_CONFIG`. **Six claims earned ✅** -- `T-033`, `T-035`, `T-036`, `T-037`, `T-038`, `T-039` -- each on a measurement the auditor performed, with every mutation restored byte-exact. **Two are REOPENED.** `T-034`: the gate widening is real (14 -> 28 entries = 56 checks, dark worst 4.62, every pre-fix number reproduced to 2dp), but `info-on-selected` = **4.4996** and `danger-on-selected` = **4.4995** are below WCAG AA and pass only because `check_contrast.py` compares `round(ratio, 2) >= threshold`; remove the rounding and the gate goes RED on exactly those two pairs, both of which `T-034` itself added. `A-09`: routes 2 and 3 survive attack, but the *declared eight-leaf free-text register* does not -- `isContractId` admits 128 characters of `[a-z0-9._-]`, so a 64-hex credential in `taskId` rides through `contract_draft.task_id` with the register reporting nothing and the sweep silent, and **deleting three of the eight declared entries leaves all 10 tests green** because the single fixture never populates them. **`T-023` is not promoted:** the ACE dump and the `icacls` harness fix are real and were *read, not run* -- the defect lives on the GitHub runner's inherited `_temp` ACL, no local box reproduces it, and the job is in `deliberately_excluded`, so nothing can distinguish "fixed" from "has not recurred yet". **13 new findings**, `I-01`..`I-13` (P2 6 / P3 7), including a machine mirror whose prose drifted three PR generations while its gate stayed GREEN, a roadmap board disagreeing with its own checkboxes on exactly the two phases said to be blocked, and two Phase-10 `contracts/` boxes that no production gate blocks. **RED is the standing verdict of record and the gate stays shut.** The index is `apps/desktop/AUDIT/AUDIT_LEDGER.md`. Superseded: the EIGHTH round (`2026-08-18-eighth-audit-9ae2fd2.md`, RED, no P0, 27 promotions) and the FOURTH round -- `apps/desktop/AUDIT/2026-08-15-zero-trust-reaudit-0a9a1af.md`, a re-audit of the third round's five fixes against a **pinned snapshot** of `main` @ `0a9a1af` (the auditor proved the pin: `rev-parse 0a9a1af^{tree}` == its own `write-tree`, because main moved three times mid-run) -- could **not reopen four of the five**. `B-01`: the fifth, `A-01`, was fixed on Python/Linux only while this ledger's row claimed **both platforms** -- the F-02 pattern the ledger exists to catch. Closed on Windows 2026-08-15. `B-02` (the pin sits in the authority, not the supervisor that owns the floor) stays **OPEN** as a topology question beside the 1b decision. Superseding: the THIRD independent audit -- `apps/desktop/AUDIT/2026-08-14-zero-trust-audit-e0dd969.md`, of `main` @ `e0dd969`, auditor-role-only and READ-ONLY on the tree -- raised **5 new findings** (A-01..A-05, P2 1 / P3 4), **could not reopen the previous round's P0** on either platform, and **confirmed all three of the gate's refusals closed** at that head. It attacked 14 Builder claims and could not refute **9**, which it recommends for the independently-confirmed mark; it also found **4 ledger rows stale** and **2 false**. Its headline is **A-01**: the anti-rollback floor is scoped by `install_id`, which the broker chooses -- the R-07/R-10 bootstrap defect surviving one level up rather than closing, on both platforms, demonstrated against the repository's own ledger code. **RED is the standing verdict of record and the gate stays shut.** The index is `apps/desktop/AUDIT/AUDIT_LEDGER.md`; the superseded round is `2026-08-06-remediation-audit.md` (45 findings, 1 P0, at `219c763`).
>
