...
**Last updated · Վերջին թարմացум:** 2026-07-27 (Phase 0 repository-truth MERGED `b6c6712`; PR #31 rebased onto the repaired main; Wave 3b-1B rev-26 audit prep).
...
> `CURRENT_ACTIVE_TASK: T-017` · `CURRENT_ACTIVE_WAVE: 3b-1B` · `CURRENT_PHASE0: done` · `CURRENT_DESIGN_GATE: PENDING_REAUDIT` · `CURRENT_DESIGN_PR: 31` · `CURRENT_IMPL_PR: 32` · `CURRENT_IMPL_STATE: WIP` · `CURRENT_PRODUCTION_VERIFIED: false` · `CURRENT_VERIFY_SEAM: complete` · `CURRENT_RECEIPT_PLUMBING: complete` · `CURRENT_GOVERNED_ROUNDTRIP: complete`
>
> Wave 3a slices 1–3 are **merged, zero-trust GREEN** — the verify-seam, receipt-plumbing, and the real fail-closed governed round-trip **all landed** (PR #28). Wave 3b-0 design **merged** (PR #30). **Phase 0 (repository-truth remediation) is DONE — merged via PR #33** (Owner-approved GREEN at exact `45f3793`, squash **`b6c6712`**; the Phase-0 carrier machinery is retired). **PR #31** (`feat/wave-3b1-isolated-signer`, **rebased onto the repaired `main` `b6c6712`**) = 3b-1A **Code GREEN** + the rev-26 design **candidate, PENDING Architect re-audit** (not GREEN); it is the **self-carrier** of the snapshot (exact audited HEAD recorded in its PR body + `design_gate`). **PR #32** (`impl/wave-3b1b-execution-binding`, Draft) = **WIP** 3b-1B implementation (a draft checkpoint, not an RC; still needs its own rebase onto the rewritten #31 branch). **No production `trusted_verified`** (`NoTrustedManifest` fail-closed). **Open:** the remaining AI entry points + **Windows production isolation** + Phases 2–10.
>
> **Next action:** rev-26's **3 preparation-P0 are CLOSED** on this HEAD (executor-binary sha pin + non-writable TCB §2.5; distinct-UID sidecar linchpin §2.6; Windows normative + enforced fail-closed platform gate §0.1/§0.W) — **submit rev-26 for the Architect design audit at PR #31's exact HEAD**. Preparation-review closure is NOT the Architect's verdict; do **not** merge PR #31 until rev-26 is Architect design-GREEN at the exact HEAD.
>
...
1. **Wave 3b — isolated signer + signed manifest + production "Verified" (T-017)** — fill the
   ...
   rev-26 design candidate PENDING re-audit; WIP implementation in PR #32 (Draft, not an RC). **Next
   permitted action:** remediate rev-26's **3 preparation-P0** (executor-binary sha pin + non-writable
   TCB; distinct-UID sidecar provisioning; Windows normative design + enforced fail-closed platform gate)
   on PR #31, then **submit rev-26 for the Architect design audit at PR #31's exact rebased HEAD**. No
   merges until an exact-head verdict; `NoTrustedManifest` stays fail-closed (no production "Verified" yet).
