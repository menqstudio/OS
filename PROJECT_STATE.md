...
> **✅ SETTLED — `main` is at `5405abd`.** The pull request that records it is PR #114 on `gate-deadlock-parked-pr`. Also open, and deliberately not merged here: PR #112 (`design/floor-writer-service`). Start from `docs/OWNER_ACTION_REQUIRED.md`, the one page that says what is blocked and on whom.
>
> **Next:** PR #112 (floor-writer service design, T-020) is OPEN and parked on purpose: it is a section-I design proposal and the Builder does not merge those. It is recorded in prs[] and exact-head anchored, so if it moves this snapshot goes RED. The open question is WHO reviews it, since there is no Architect - the same problem the independent audit had, solved there by a separate clean-context auditor session; if that route is used the verdict must be recorded as what it is, not as Architect GREEN (OWNER_APPROVED_NOT_ARCHITECT_AUDITED exists for that). Also open: B-02 - the anti-rollback floor has THREE claimed owners - and the eighth principal amends the rev-30 addendum's normative 2.5/2.6 SEVEN runtime service UIDs, which FW-3 is blocked on.
>
