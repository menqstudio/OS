...
>
> **▶ CURRENT STATE (2026-07-27, authoritative — supersedes any "next is slice 3" wording above):** Wave 3a slices 1–3 are **merged, zero-trust GREEN** (verify-seam + receipt-plumbing + the real fail-closed governed round-trip **all landed** via PR #28 — none are "pending"). **Wave 3b-0** design **merged** (PR #30, `df3c0ac`). **PR #31** (`feat/wave-3b1-isolated-signer`) = 3b-1A **Code GREEN** + the 3b-1B **rev-26 design candidate (PENDING re-audit, not GREEN)**. **PR #32** (`impl/wave-3b1b-execution-binding`, Draft) = **WIP 3b-1B implementation** (not merged, not an RC). **No production `trusted_verified`** yet (`NoTrustedManifest` fail-closed). **Still open:** governing the remaining AI entry points (run-steps, Ask Bro, conversation-reply, automations, group chat, integrations), **Windows production isolation**, and Phases 2–10 of the product roadmap. Machine mirror: [`config/current_state.json`](./config/current_state.json). **Exact handoff: [`NEXT_CHAT.md`](./NEXT_CHAT.md).**
...
1. **Wave 3b — isolated signer + signed manifest + production "Verified" (T-017)** — fill the
   ...
   progress** — branch `feat/wave-3b1-isolated-signer` exists (PR #31, HEAD `6ebeca8`): 3b-1A code
   Architect Code GREEN; 3b-1B rev-26 design NOT-GREEN; WIP implementation in PR #32 (Draft, not an RC).
   Next permitted action: submit PR #31 rev-26 for a fresh exact-head Architect design audit. No merges
   until an exact-head verdict; `NoTrustedManifest` stays fail-closed (no production "Verified" yet).
