...
> **✅ SETTLED — `main` is at `0a9a1af`.** Nothing is open. Start from `docs/OWNER_ACTION_REQUIRED.md`, the one page that says what is blocked and on whom.
>
> **Next:** Phase 2 is now workable under a committed exemption. Its eleven boxes: build the governance read IPC (ledger/evidence/queue) in Rust with parse tests, then the four pages to full section-D spec - approvals (queue + grant/deny/escalate REQUEST, never a decision), decisions (append-only ledger + evidence viewer, read-only), security (chain integrity + control-plane digest + residual tracker), notifications (signal feed) - plus a contract test that an approval-request carries no key or lease and that verdicts render faithfully. No desktop-side decision authority, no cached keys or leases. In parallel the Owner is commissioning a RE-AUDIT of the third audit's five fixes; it should pin the exact head it assesses, and Phase 2 commits land after it.
>
