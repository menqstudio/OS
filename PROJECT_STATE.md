...
> **✅ SETTLED — `main` is at `dc8d867`.** The only thing open is PR #113 on `canon-truth-after-112`, the pull request that records it. Start from `docs/OWNER_ACTION_REQUIRED.md`, the one page that says what is blocked and on whom.
>
> **Next:** PR #112 is OPEN and the Builder does not merge it: it is a section-I design proposal (floor-writer service, T-020) and the precedent for a design PR here is #30, merged only after an Architect DESIGN GREEN. The open question is WHO reviews it, since there is no Architect - the same problem the independent audit had, solved there by a separate clean-context auditor session; if that route is used the verdict must be recorded as what it is, not as Architect GREEN (OWNER_APPROVED_NOT_ARCHITECT_AUDITED exists for that). Also open: B-02 in full - the anti-rollback floor has THREE claimed owners (addendum 7 P1-7 and supervisor_ledger.rs:20 say brops-signer; the DDL, the CAS and the only process opening the file say supervisor; the scope-key pin is the challenge authority's) - and the eighth principal amends the rev-30 addendum's normative 2.5/2.6 SEVEN runtime service UIDs, which FW-3 is blocked on.
>
