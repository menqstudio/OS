...
**Last updated · Վերջին թարմացум:** 2026-07-27 (Phase-0 repository-truth correction round 2).
...
>
> Wave 3a slices 1–3 are **merged, zero-trust GREEN** — the verify-seam, receipt-plumbing, and the real fail-closed governed round-trip **all landed** (PR #28). Wave 3b-0 design **merged** (PR #30). **PR #31** (`feat/wave-3b1-isolated-signer`) = 3b-1A **Code GREEN** + the rev-26 design **candidate, PENDING re-audit** (not GREEN). **PR #32** (`impl/wave-3b1b-execution-binding`, Draft) = **WIP** 3b-1B implementation (not merged, not an RC). **PR #33** (`chore/phase0-repository-truth`) = repository-truth candidate **PENDING re-audit** (not merged). **No production `trusted_verified`** (`NoTrustedManifest` fail-closed). **Open:** the remaining AI entry points + **Windows production isolation** + Phases 2–10. **Correct next sequence:** PR #33 re-audit GREEN → merge → verify main → rebase #31 → new exact HEAD → remediate rev-26 preparation-P0 → exact-head Architect design audit on the new HEAD. Machine mirror: [`config/current_state.json`](./config/current_state.json).
<!-- CURRENT_STATE_END -->
...
1. **Wave 3b — isolated signer + signed manifest + production "Verified" (T-017)** — fill the
   ...
   (corrected sequence):** re-audit + merge PR #33 (repository truth) → verify new `main` → rebase PR #31
   onto it → resolve PR #31's NEW exact HEAD → remediate rev-26's 3 preparation-P0 → submit rev-26 for the
   Architect design audit **on that new HEAD** (NOT the pre-rebase `6ebeca8`). No merges until an
   exact-head verdict; `NoTrustedManifest` stays fail-closed (no production "Verified" yet).
