...
**Last updated · Վերջին թարմացум:** **Wave 3a slice 2 (receipt storage & atomicity, T-015) — IMPLEMENTED, in Review** on `feat/wave-3a-receipt-storage` (cut off `main` @ `75a8d8f`): migration **0014** (`SCHEMA_VERSION`=14 — `receipt_verification_attempts` with `wire_*` + decoded evidence and DB-level accepted⇔message / blocked⇔no-message CHECK, durable one-time `receipt_challenges` nonce, accepted-only `receipt_ids_seen` uniqueness ledger) + `brops-core::receipt_store` (`verify_and_record_receipt` = one `BEGIN IMMEDIATE` verify→consume→persist; `issue_challenge`; `ReceiptOutcome` has **no `TrustedVerified` variant** — production⇒Blocked). Architect **YELLOW** fixes applied, then **audit round 1 RED (4 blockers on `24869eb`) RESOLVED**: challenge `request_sha256` NOT-NULL+hex and compared in-tx to the Expected; staged decoded evidence on bad-sig/bind-fail; `message_id` `ON DELETE SET NULL` so accepted evidence survives conversation/message deletion; nested-tx invocation rejected + explicit COMMIT-failure rollback. **83 core tests** (14 slice-2 negative-matrix), clippy-clean, coordination + capabilities GREEN. **Awaiting re-audit on the new pushed HEAD + Owner merge; NOT merged.** Prior: **Wave 3a slice 1 (protocol core) — DONE, MERGED** (T-014, PR #24). Approved HEAD `c51031e`, squash **merge commit `6c920d0`** on `main`; **zero-trust GREEN** after three RED rounds (key-authority binding, `Wave3aTrustState` with no `TrustedVerified` variant, `IssuedRequest` request-hash recompute — all resolved audit history); final CI 7/7 GREEN; `brops-core` **69 tests**, clippy-clean. Slice 1 shipped the pure, I/O-free `brops-core::receipt` (RFC 8785 JCS, strict decode, verify-only `verify_strict`, type-state `parse→verify→bind→resolve_3a`, never a `sign()` oracle). **Next: Wave 3a slice 2 — receipt storage & atomicity** (migration 0014: `receipt_verification_attempts` + durable one-time nonce + `receipt_id` uniqueness; atomic verify→consume→persist; freshness/skew) — **not started; cut `feat/wave-3a-receipt-storage` from main.** Transport + UI = slice 3; isolated signer + manifest + production "Verified" = Wave 3b. **Wave 2 (T-010 + T-011) + Wave 1 (T-012) + Wave 2a (T-013) complete.** **Exact handoff: [`NEXT_CHAT.md`](./NEXT_CHAT.md).**
...
|---|---|---|---|
| 🔨 Claude | Wave 3a — Receipt Protocol v1 **slice 2 (receipt storage & atomicity)** (T-015) — migration 0014 + atomic verify→consume→persist | `feat/wave-3a-receipt-storage` (PR #26) | 🔎 **Review** — **Audit round 1 RED (4 blockers on `24869eb`) RESOLVED** (challenge request_sha256 in-tx compare; staged decoded evidence; `ON DELETE SET NULL` evidence-survives-deletion; nested-tx rejected + commit-failure rollback); **83 core tests** green, clippy-clean, coordination+capabilities GREEN; **awaiting re-audit on the new pushed HEAD + Owner merge** |
| 📐 ChatGPT | — | — | — |
