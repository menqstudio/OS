...
> **✅ SETTLED — `main` is at `e0dd969`.** The only thing open is PR #99 on `record-2026-08-14-audit`, the pull request that records it. Start from `docs/OWNER_ACTION_REQUIRED.md`, the one page that says what is blocked and on whom.
>
> **Next:** Apply the third audit's outcome. FIRST in AUDIT_LEDGER.md: promote the 9 rows section 3 of the report names (copy the auditor's list, do not summarise it), correct the 2 rows found false, refresh the 4 found stale. THEN the findings, A-01 first: pin install_id from deployment config the way governed_acceptance.py:405 pins supervisor_id, and refuse a request whose install_id disagrees. Then A-02, A-05, A-03/A-04. Also open: Owner decision 1b.
>
