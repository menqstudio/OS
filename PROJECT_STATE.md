...
> **✅ SETTLED — `main` is at `f652d37`.** Nothing is open. Start from `docs/OWNER_ACTION_REQUIRED.md`, the one page that says what is blocked and on whom.
>
> **Next:** B-01 is closed on Windows and B-03 by the re-auditor's own confirmation. B-02 stays OPEN and is deliberately not a Builder edit: the install_id pin lives in the authority while the supervisor owns the floor - a topology question sitting beside the 1b decision, since a floor-writer service that owns the marks directory would be the natural holder of the pin too. Section I change-control. Still unprotected, as the A-05 row admits: verifier is JCS, writers are serde, they agree on real events, and no test feeds a Linux-written chain to the Windows parser. Phase 2 is unlocked under a committed exemption; all four of its pages already exist, so its boxes need checking against the code before anything is built.
>
