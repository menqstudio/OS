...
|---|---|---|---|
| 🔨 Claude | Wave 3a — Receipt Protocol v1 **slice 3 (transport wiring + receipt trust UI)** (T-016) | `feat/wave-3a-transport-ui` (PR #28) | 🔎 **Review** — **IMPLEMENTED** (candidate `32a71d1`): key-authority seam (resolver in-tx, no fake key), `Message.receipt` projection, bridge=transport/desktop=authority (`verified` removed, wire carried), ai.rs+commands.rs governed branch (issue_challenge→verify, no double-post, `StreamEvent::Blocked` notice), frontend badge+notice, JCS parity + e2e. **Audit rounds 1 (5+hardening) + 2 (3) — RED — RESOLVED:** R1 (challenge context to signer, exact bytes, collision-safe history, fresh verify clock, real transport reason, fail-closed resolve); R2 (one `PreparedGovernedTurn` single source for trim+hash+context; structured `system`+`history` are the bridge signing authority; bounded transport-failure evidence). **core 89 · host 42 · bridge 35 py · frontend 6** green, clippy-clean, coordination+capabilities GREEN. **Awaiting round-3 re-audit + Owner merge.** Slice 2 (T-015) done+merged (`9b214e5`) |
| 📐 ChatGPT | — | — | — |
...
1. **Wave 3a slice 3 — transport wiring + receipt trust UI (T-016)** — the layer that makes the desktop
   CALL the merged verifier on a real governed turn: issue the desktop nonce/challenge on send
   (`receipt_store::issue_challenge`), route the returned receipt through
   `receipt_store::verify_and_record_receipt` (adapter → injected verifier, desktop = final authority),
   sign-on-complete buffering, persist via the atomic tx, and render **dev/blocked** trust badges (never
   "Verified"); one real governed round-trip e2e. **Not started** — cut a fresh branch from `main`
   @ `9b214e5`, claim T-016. (Isolated signer + manifest + production "Verified" = Wave 3b.)
