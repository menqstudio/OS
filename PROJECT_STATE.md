...
> **✅ SETTLED — `main` is at `0a9a1af`.** The only thing open is PR #107 on `phase2-exemption`, the pull request that records it. Start from `docs/OWNER_ACTION_REQUIRED.md`, the one page that says what is blocked and on whom.
>
> **Next:** Phase 2 is workable under a committed exemption. Its eleven boxes: the governance read IPC (ledger/evidence/queue) in Rust with parse tests, then approvals, decisions, security and notifications to full section-D spec, plus a contract test that an approval-request carries no key or lease. No desktop-side decision authority. In parallel the Owner is commissioning a RE-AUDIT of the third audit's five fixes - four of five are the Builder's own claim - and it should pin the exact head it assesses; Phase 2 commits land after it.
>
