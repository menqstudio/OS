...
**Last updated · Վերջին թարմացum:** 2026-07-27 (Phase 0 MERGED `b6c6712`; PR #31 rebased; rev-28 = Architect design **RED** (3 P0 + 1 P1 @ `c9680f5`); **rev-29** remediation submitted for re-audit).
...
> `CURRENT_ACTIVE_TASK: T-017` · `CURRENT_ACTIVE_WAVE: 3b-1B` · `CURRENT_PHASE0: done` · `CURRENT_DESIGN_GATE: PENDING_REAUDIT` · `CURRENT_DESIGN_CANDIDATE: rev-29` · `CURRENT_LAST_REVIEWED: rev-28` · `CURRENT_LAST_VERDICT: RED` · `CURRENT_DESIGN_PR: 31` · `CURRENT_IMPL_PR: 32` · `CURRENT_IMPL_STATE: WIP` · `CARRIER_IF_OPEN_GATE: PR31_DESIGN_AUDIT` · `CARRIER_IF_MERGED_GATE: VERIFY_MAIN_REBASE_PR32` · `CURRENT_PRODUCTION_VERIFIED: false` · `CURRENT_VERIFY_SEAM: complete` · `CURRENT_RECEIPT_PLUMBING: complete` · `CURRENT_GOVERNED_ROUNDTRIP: complete`

> **▶ rev-29 UPDATE (current).** The **rev-28** candidate was **Architect design RED — 3 P0 + 1 P1** at exact HEAD `c9680f5` (CI run 30280738223 9/9 GREEN; CI ≠ design GREEN). **rev-29 remediates all four and is re-submitted, PENDING_REAUDIT** — P0-1 propagate the renderer → trusted verifier/BROKER service → challenge-authority split through the ENTIRE normative contract (topology diagram + §2.1 + §4.10(g) `governed_turn_execute` + §6.1/§7.1 + a normative renderer↔broker IPC schema; authority allowlists ONLY the broker UID, DENIES the renderer; only the broker owns the receipt DB/manifest/prepared object/verification/persistence and emits the committed result; the Tauri command is a THIN proxy carrying only `{conversation_id, agent?}`); P0-2 add the broker + challenge-authority to `TCB_ARTIFACTS` (ownership + non-writability + SHA-256 pin + fail-closed verify + UID/SID + peer-auth, 7 tests); P0-3 the two-boundary FD survival contract (recorder prepares FDs 3–6 before `execve(launcher)`; launcher re-verifies before `fexecve(executor)`; 7 tests); P1-1 this canonical-state sync. The narrative below may still reference the earlier rev-27→rev-28 step; **`config/current_state.json` is the authoritative machine mirror.** Do NOT merge PR #31 until rev-29 is Architect design-GREEN at the exact HEAD.
>
> Wave 3a slices 1–3 are **merged, zero-trust GREEN** — the verify-seam, receipt-plumbing, and the real fail-closed governed round-trip **all landed** (PR #28). Wave 3b-0 design **merged** (PR #30). **Phase 0 (repository-truth remediation) is DONE — merged via PR #33** (Owner-approved GREEN at exact `45f3793`, squash **`b6c6712`**; the Phase-0 carrier machinery is retired). **PR #31** (`feat/wave-3b1-isolated-signer`, **rebased onto the repaired `main` `b6c6712`**) = 3b-1A **Code GREEN** + the 3b-1B design addendum which is **Architect design RED** — **rev-27 got 2 P0 + 4 P1** at `0e41ef6`; **rev-27** remediates and is re-submitted. PR #31 is the snapshot's **current_workflow_pr**, exact-head-anchored by its PR-body **`AUDIT_CANDIDATE_HEAD`** marker (nothing is exempt from exact-head verification). **PR #32** (`impl/wave-3b1b-execution-binding`, Draft) holds **UNAPPROVED Draft/WIP** 3b-1B code with **no authority over the design** (not an RC, not Architect-approved; adapt only after design-GREEN; frozen from architectural expansion). **No Architect-approved or merged 3b-1B implementation exists. No production `trusted_verified`** (`NoTrustedManifest` fail-closed). **Open:** the 2 P0 + 3 P1 rev-26 blockers, the remaining AI entry points, **Windows production isolation**, Phases 2–10.
>
> **Next action:** the earlier 3 preparation-P0 are closed (§2.5/§2.6/§0.1/§0.W), but the **Architect verdict on rev-27 is RED**. rev-28 closes the two P0 (P0-1 renderer/trusted-broker/challenge-authority split (nine roles, seven service UIDs) + 3 threat actors; P0-2 Model A launcher (FD survival + privilege sequence)) — then **re-submit rev-28 for the Architect design audit at PR #31's exact HEAD**. Preparation-review closure is NOT the Architect's verdict; do **not** merge PR #31 until rev-27 is Architect design-GREEN at the exact HEAD.
>
...
1. **Wave 3b — isolated signer + signed manifest + production "Verified" (T-017)** — fill the
   ...
   progress** — Phase 0 (repository-truth) is DONE (PR #33 merged, `b6c6712`). PR #31
   (`feat/wave-3b1-isolated-signer`, rebased onto the repaired `main`) = 3b-1A code Architect Code GREEN +
   the 3b-1B design addendum which is **Architect design RED** (rev-27 = 2 P0 + 4 P1 at `0e41ef6`); PR #32
   holds UNAPPROVED Draft/WIP code (no design authority). **Next permitted action:** rev-28 closes the two
   P0 (P0-1 eight-principal challenge-authority topology + desktop-client/authority separation + 3 threat
   actors; P0-2 Model A root/TCB-owned setuid launcher) on PR #31, set the AUDIT_CANDIDATE_HEAD marker to
   the new exact head, then **re-submit rev-28 for the Architect design audit at PR #31's exact HEAD**. No
   merges until an exact-head design-GREEN verdict; `NoTrustedManifest` stays fail-closed (no "Verified").
