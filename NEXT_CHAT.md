...
- **Exact current HEAD:** the **rev-25 (CONSOLIDATED)** design-lock candidate is committed locally and pushed by Claude this cycle (push/PR-write are Owner-delegated to Claude, §3.3). The immediately-preceding **rev-24** design was Architect-reviewed at exact HEAD **`232be53a64e9afccec5338d5f1440f76fae69884`** (exact-head CI **#131** 8/8 SUCCESS incl. both mandatory Wave-3b gates — CI GREEN ≠ design GREEN). **Resolve the live tip yourself:** `gh pr view 31 --json headRefOid -q .headRefOid` (or `git rev-parse HEAD` after `git pull`), then query GitHub Checks for THAT exact SHA (`gh api repos/menqstudio/OS/commits/<sha>/check-runs`). **GitHub Checks is the CI authority — never trust a hardcoded run number, and CI GREEN ≠ design/Architect GREEN.**
- **Where it stands:** **3b-1A** = ✅ Architect Code GREEN (`dffd164`). **3b-1B** = ❌ Architect **Design RED** — the Architect reviewed **rev 24** @ `232be53` (exact-head CI **#131** 8/8 SUCCESS; CI GREEN ≠ design GREEN), **CONFIRMED CLOSED** the rev-23 single-source-constructibility P0 + authority-failure-boundary P1, and returned RED with **1 P0 · 1 P1** (**P0** model identity had two independent authorities — the desktop `generation_config` the executor reads via FD vs the lease's `model_profile_id`, unbound, so a signed lease could declare "profile A" over a "model B" config; **P1** the ISSUED authority-row cleanup was contradictory — rev 24 said `PENDING_TTL` sweeps a terminal ISSUED row that must retain its signed document for replay); **rev 25** closes both in place (§2/§4.3/§7 + §2.1) via a read-only real-code investigation + one integrator + a fresh independent red-team. rev 25 is a **PROPOSED** design-GREEN candidate — **NOT Architect-GREEN.** **No 3b-1B / 3b-2 / 3b-3 code exists.**
- **Your next action:** rev 25 is pushed by Claude + the PR #31 body refreshed; submit **rev 25** (`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`, the single normative source) for Architect design review **at the exact resolved HEAD**. If RED → run the fan-out/integrator/red-team loop and revise in place; if GREEN → only then write 3b-1B code. Do **not** merge PR #31, do **not** start 3b-2/3b-3, do **not** claim Architect-GREEN, do **not** touch `NoTrustedManifest` or expose production "Verified". **Push/PR-write are Owner-delegated to Claude (see §3.3); merge + final design approval remain the Owner's (Gev). The AI never merges.**
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 25 (CONSOLIDATED)** — **the single normative source for every 3b-1B contract** (artifact matrix §3, exact schemas §4 incl. the control-plane §4.10 + the desktop→sidecar ingress + preparation contract + `governed_turn_execute`/`GovernedTurnExecutionV1B` + `resolve_governed_generation_config_v1b()` + authority-channel failure boundary §4.10(g), ms time model §1, capability profile §2 + one-model-authority registry §2.2 + challenge-creation channel + pending/issued retention §2.1 + store ACL §2.3 + bounded ingress §2.4, durable acceptance state machine §5, atomic order + E2E §6/§6.1, verification §7 incl. config↔profile↔lease binding + desktop-signatures-only §7.1, authorities §8; revision history in non-normative Appendix A). History: REDs rev 6→…→23 (Architect RED @ `b89bab9`, CI #130, 1 P0 · 1 P1) → rev 24 → the Architect **reviewed rev 24 at HEAD `232be53` (exact-head CI #131 8/8 SUCCESS; CI GREEN ≠ design GREEN), CONFIRMED CLOSED the rev-23 single-source-constructibility P0 + authority-failure-boundary P1, and returned Design RED with 1 P0 · 1 P1 (P0 model identity had two independent authorities — the desktop `generation_config` vs the lease's `model_profile_id`, unbound; P1 the ISSUED authority-row cleanup was contradictory), mandating a read-only real-code investigation + one integrator + a FRESH independent red-team (NOT single-context guessing, NOT a rewrite).** **rev 25 applies both corrections in place; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The two rev-24 → rev-25 findings (closed in place but treat as OPEN until the Architect returns design-GREEN):** *(the rev-18 → … → rev-23 findings — orchestrator ordering, generation_config canonicalization, two-trust-model channel, nonce/`request_sha256` decoupling, generation_config hash-source split, prepared-object lifecycle, routing identities, non-durable transport-retry, single-source-constructibility, authority-failure-boundary — are CONFIRMED CLOSED by the Architect; now history in the addendum's non-normative Appendix A.)*
  1. **P0 — model identity had two independent authorities.** The desktop resolver's `generation_config` (engine_id/model/…, read by the executor via FD, §4.7) and the lease's `model_profile_id` were unbound — a signed lease could declare "profile A" while the executor ran "model B": cryptographically valid but a misrepresented execution identity. **rev 25 (§2.2, §4.3, §7):** the supervisor owns a **deterministic model-profile registry** `SHA256(JCS(generation_config)) → model_profile_id` (total over the accepted governed configs; unknown ⇒ `model_profile_unknown` Block, no lease); **before acceptance** the supervisor parses/validates the staged config, recomputes `generation_config_sha256`, and **derives** `model_profile_id`; `issue_governed_turn_lease` sets the lease `model_profile_id` = derived and binds `generation_config_sha256` (a signed lease field); `validate_governed_turn_lease` + `LiveRunStateProvider` (§7) re-derive and refuse unless `lease.model_profile_id == registry[lease.generation_config_sha256]` and `lease.generation_config_sha256 == challenge.generation_config_sha256 == the executed config's hash` — every model/engine/parameter mismatch Blocks **before launch**.
  2. **P1 — the ISSUED authority-row cleanup was contradictory.** rev 24 said `PENDING_TTL` sweeps a left-behind `ISSUED` row, but `ISSUED` is terminal and retains the exact signed document for byte-identical replay — only `PENDING`-row expiry was defined. **rev 25 (§2.1):** `PENDING` rows expire at `PENDING_TTL_MS`; an `ISSUED` row is retained for **`ISSUED_RETENTION_MS = challenge_expiry_window + AUTHORITY_REPLAY_WINDOW_MS`** then swept (document + row); a per-`install_id` `MAX_ISSUED_ROWS_PER_INSTALL` / `MAX_ISSUED_BYTES_PER_INSTALL` quota bounds stored issued documents; a deterministic `PENDING_SWEEP_INTERVAL_MS = 60000` sweep removes expired PENDING + retention-expired ISSUED rows; post-`ISSUED_RETENTION_MS` replay ⇒ `no_pending_row`. The §4.10(g) "PENDING_TTL sweeps ISSUED" wording is corrected.
- **Independent red-team (fresh, not the integrator):** one adversarial pass over the full rev-25 working-tree diff + the real repo code; returned **no BLOCKER** — the config↔profile↔lease binding verified against the §4.3 lease schema + §7 provider + the §4.5 `GOVERNED_REFUSAL_REASONS` (added `model_profile_unknown`), and the pending/issued retention against §2.1; the frozen 3b-1A/Wave-3a path proven untouched (not in the diff), and `check_coordination` + `check_capabilities` GREEN live. Any lower-severity notes were fixed before commit.
...
- **Next permitted action:** submit rev 25 @ the pushed HEAD (resolve the live tip from GitHub) for Architect design review; if RED, run the fan-out/integrator/red-team loop, revise in place, commit+push+refresh-PR-body, and re-submit; if GREEN, only then write 3b-1B **code** (after Owner "go").
...
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on the consolidated rev 24 (@ `232be53`, CI #131 8/8 SUCCESS ≠ design GREEN; rev-23 single-source P0 + authority-failure-boundary P1 CONFIRMED CLOSED; new 1 P0 · 1 P1 two-model-authorities + issued-row-cleanup); **rev 25 CONSOLIDATED proposed, not yet GREEN, no code** — §3.3 |
