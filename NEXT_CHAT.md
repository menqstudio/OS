...
- **Exact current HEAD:** the **rev-19 (CONSOLIDATED)** design-lock candidate is being pushed in this commit; the immediately-preceding **rev-18** candidate was Architect-reviewed at exact HEAD **`89d0df4c7211c97c85c582090cea05c5da02bc42`**, whose **exact-head CI #124 is 8/8 GREEN** (both mandatory gates `Engine · governance runtime` + `Engine · signer isolation proof`, plus the `Coordination · docs consistency gate`). **The live tip may be one doc-only commit ahead of that SHA** — always resolve it yourself: `gh pr view 31 --json headRefOid -q .headRefOid` (or `git rev-parse HEAD` after `git pull`), then query GitHub Checks for THAT exact SHA (`gh api repos/menqstudio/OS/commits/<sha>/check-runs`). **GitHub Checks is the CI authority — never trust a hardcoded run number, and CI GREEN ≠ design/Architect GREEN.**
- **Where it stands:** **3b-1A** = ✅ Architect Code GREEN (`dffd164`). **3b-1B** = ❌ Architect **Design RED** — the Architect reviewed **rev 18** @ `89d0df4c` (exact-head CI **#124** SUCCESS 8/8 — CI GREEN ≠ design GREEN) and returned RED with **1 P0 + 2 P1** (orchestrator-ordering / generation_config-canonicalization / challenge-creation-channel); **rev 19** applied all three fixes in place (see §3.3) via a read-only fan-out + one integrator + a fresh independent red-team. rev 19 is a **PROPOSED** design-GREEN candidate — **NOT Architect-GREEN.** **No 3b-1B / 3b-2 / 3b-3 code exists.**
- **Your next action:** submit **rev 19** (`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`, the single normative source) for Architect design review **at the exact resolved HEAD**. If RED → run the fan-out/integrator/red-team loop and revise in place; if GREEN → only then write 3b-1B code. Do **not** merge PR #31, do **not** start 3b-2/3b-3, do **not** claim Architect-GREEN, do **not** touch `NoTrustedManifest` or expose production "Verified".
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 19 (CONSOLIDATED)** — **the single normative source for every 3b-1B contract** (artifact matrix §3, exact schemas §4 incl. the control-plane §4.10 + the desktop→sidecar ingress §4.10(g), ms time model §1, capability profile §2 + challenge-creation channel §2.1 + protocol-versioning §2.2 + store ACL §2.3 + bounded ingress §2.4, durable acceptance state machine §5, atomic order + E2E §6/§6.1, verification §7 + desktop-signatures-only §7.1, authorities §8; revision history in non-normative Appendix A). History: REDs rev 6→…→17 (Architect RED @ `d8a6510`, CI #121, 2 P0 + 3 P1) → rev 18 → the Architect **reviewed rev 18 at HEAD `89d0df4c` (exact-head CI #124 SUCCESS 8/8 — CI GREEN ≠ design GREEN), and returned Design RED with 1 P0 + 2 P1 orchestrator-ordering / generation_config-canonicalization / challenge-creation-channel findings, mandating a read-only fan-out audit + one integrator + a FRESH independent red-team (NOT single-context guessing, NOT a rewrite).** **rev 19 applies those corrections in place via that fan-out; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The three rev-18 → rev-19 findings (closed in place but treat as OPEN until the Architect returns design-GREEN):** *(the prior rev-17 → rev-18 five-finding closure is now history — see the addendum's non-normative Appendix A.)*
  1. **P0-1 — the §4.10(g) sidecar orchestrator ordering was impossible + the submit subprocess pulled output.** The orchestrator sequenced `staging×3 → turn-open → evidence-request → result → PULL output`, but `brops.governed-staging-open.v1` requires the `UPLOADING` `governed_turn_staging` row that ONLY `brops.governed-turn-open.v1` creates (else `no_staging_row`), so no turn could ever execute; and the submit subprocess must not pull output. **rev 19 (§4.10(g), §6.1):** the submit subprocess is reordered to **`turn-open → staging×3 → evidence-request → metadata-result → reframe → exit`** and performs **NO `brops.governed-turn-output-read.v1` PULL** — the output pull is confined to the separate §6.1 step-13/14 desktop-driven `governed_turn_output_read` subprocess; the submit subprocess emits exactly one metadata-only `bridge.governed-turn-result.v1` and exits. Adds an exact call-order test (turn-open MUST precede the first staging-open; staging-open before turn-open ⇒ `no_staging_row`).
  2. **P1-2 — the challenge-authority creation channel left a choice between two trust models.** §2.1 accepted "structured turn facts **OR** a protected row-id" while §4.1 required "row-id only" — a design-lock cannot leave the implementer that choice. **rev 19 (§2.1, §4.1):** collapsed to a **single protected-row-ID model with two explicit messages** — `brops.governed-challenge-create-pending.v1` (the desktop-UI's structured turn facts are validated and stored in the authority's OWN `0700` pending-challenge row; no signing) then `brops.governed-challenge-issue.v1` (the desktop-UI supplies **only** the `pending_challenge_id`; the authority builds the exact §4.1 payload from its own row and signs once, one-time-consume). Full request/reply + pending-row schemas, validation, idempotency, and 4 KiB frame limits; the single invariant is that the caller never supplies challenge bytes and the authority always builds the payload from its own row.
  3. **P1-1 — `generation_config_bytes = JCS(object)` relied on representation-ambiguous JCS numeric canonicalization.** The real `receipt.rs::jcs_bytes:235-237` is a `BTreeMap<String,String>`-only serializer (no numeric/general-RFC-8785 path exists), and Python `json.dumps` float-repr ≠ Rust `ryu` ≠ RFC 8785 ECMAScript number formatting — so a legitimate numeric config could be committed by the Rust authority and re-hash differently on the Python sidecar → false `handle_not_challenge`/`sha_mismatch` Block. **rev 19 (§2.2, §4.10(g)):** `generation_config` is now a **flat string→string object** — `temperature`/`top_p` fixed-point decimal strings (exactly 2 fractional digits) + `max_output_tokens` a canonical decimal-integer string, validated by regex + integer-range checks that reject exponent/`-0.0`/precision/bare-int forms **before** canonicalization — so `JCS(generation_config)` rides the already-proven string→string primitive and no number is ever serialized; strictly additive to the frozen 3b-1A string `generation_config_bytes` + its `receipt.rs:1216-1219` parity fixture + `prepare_governed_turn`'s `&str` signature (§2.2 KEEP+ADD, untouched).
- **Independent red-team (fresh, not the integrator):** one adversarial pass over the full rev-19 working-tree diff + the real repo code; returned **no BLOCKER** — every P0/P1 closure verified against the real `receipt.rs::jcs_bytes` / `brops_canonical.py` / the staging + challenge contracts, the frozen 3b-1A family proven untouched (not in the diff), and the `check_coordination` manifest gate + its tests run GREEN live. Any lower-severity notes were fixed before commit.
...
- **Next permitted action:** submit rev 19 for Architect design review at the exact pushed HEAD; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on the consolidated rev 18 (@ `89d0df4c`, CI #124 SUCCESS ≠ design GREEN; 1 P0 + 2 P1); **rev 19 CONSOLIDATED proposed, not yet GREEN, no code** — §3.3 |
