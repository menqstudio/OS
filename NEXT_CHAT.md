...
- **Exact current HEAD:** the **rev-22 (CONSOLIDATED)** design-lock candidate is committed locally (this cycle Claude does NOT push — hands the exact push + `gh pr edit` commands to Gev, see §3.3; the PR #31 body still shows **rev 19** — last two cycles Gev pushed without the `gh pr edit`, so the rev-22 handoff's `gh pr edit` brings it current); the immediately-preceding **rev-21** candidate was Architect-reviewed at exact HEAD **`a05629b7179e9ee87f315e5ac8452e88c8f4f89a`** (exact-head CI **#127** — the two mandatory gates `Engine · governance runtime` + `Engine · signer isolation proof` + the `Coordination · docs consistency gate` SUCCESS; the `Cockpit · Tauri host` job was still running at review, which does not change the verdict — CI GREEN ≠ design GREEN). **Resolve the live tip yourself** after Gev pushes: `gh pr view 31 --json headRefOid -q .headRefOid` (or `git rev-parse HEAD` after `git pull`), then query GitHub Checks for THAT exact SHA (`gh api repos/menqstudio/OS/commits/<sha>/check-runs`). **GitHub Checks is the CI authority — never trust a hardcoded run number, and CI GREEN ≠ design/Architect GREEN.**
...
- **Push/PR-write mode (canonical):** this cycle Claude committed **locally only** and hands Gev the exact `git push` + `gh pr edit` commands; Gev pushes / updates the PR / merges. OWNERS gives Gev final approval/merge authority; the AI does not push or merge. **Coordination note:** the PR #31 body has lagged at **rev-19** for two cycles (Gev pushed without the `gh pr edit`); the rev-22 handoff includes the `gh pr edit` to bring it current.
- **Next permitted action:** Gev pushes the local rev-22 commit + refreshes the PR body (currently rev-19); then submit rev 22 for Architect design review at the exact pushed HEAD; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
