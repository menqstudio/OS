...
- **Branch HEAD:** the tip of `feat/wave-3b1-isolated-signer` — confirm with `git rev-parse HEAD`. The substantive content is the **3b-1B addendum (currently rev 7)** plus the documentation-consolidation updates; the exact tip SHA moves with each doc commit. **Always re-confirm the exact HEAD from git, never from memory.**
- **Base `main`:** `df3c0aca80cbe4a5537a9fdd53e16e26541c9c19` (Wave 3b-0 design merged, PR #30).
- The branch contains: **3b-1A code** (Architect-GREEN, §3.2) + the **3b-1 re-scope map** + the **3b-1B design-lock addendum** (**rev 7**, design RED, §3.3). It carries **no 3b-1B, 3b-2, or 3b-3 code.**
...
- **Machine evidence:** exact-head CI **#106, #107, and #108** (through `027603d`) were **fully GREEN** — all **8** jobs, including both mandatory gates **`Engine · governance runtime`** and **`Engine · signer isolation proof`**. The Linux isolation job proves a **positive supervisor→signer signed round-trip BEFORE** the four same-login-user **denial** proofs, using dedicated service users. (A later run **#109** on the docs/addendum HEADs had engine, isolation, bridge, frontend, SQLite, capability and docs jobs GREEN with the Tauri job still in progress at last check — the docs-only commits do not touch 3b-1A code, so the `dffd164` GREEN stands; re-confirm #109 from CI.)
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 7**. The Architect **reviewed rev 6 and returned Design RED** (2 P0 + 3 P1 **narrow consistency** blockers — "very close to GREEN"). **rev 7 is my proposed closure of those five; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The five rev-6 → rev-7 blockers (addressed by rev 7 but treat as OPEN until the Architect returns design-GREEN):**
  1. **P0-1 — challenge-authority input path could still be a two-step oracle.** The authority reading bytes from a store the desktop populates was under-specified: if a caller could seed caller-chosen bytes then ask the authority to sign that id, a compromised sidecar would have a `create_pending(attacker_payload) → sign(pending_id)` oracle. **rev 7:** the authority **builds** the challenge from the **trusted desktop DB** (caller supplies only an ID, never bytes); the pending-challenge row is written **only by the app/package-identity-authenticated desktop host**, a channel the same-login sidecar lacks; sidecar cannot-create-pending + cannot-sign-caller-bytes are machine-proven.
  2. **P0-2 — launcher FD contract internally contradictory.** The TCB said "close all FDs but the output pipe" while the flow requires preserving read-only input FDs. **rev 7:** ONE canonical FD table — FDs `3`/`4`/`5` read-only `system`/`history`/`generation_config`, FD `6` write-only output, modes validated, every other FD closed.
  3. **P1-3 — nobody published the signed challenge document.** The record binds `challenge_handle` and the provider re-fetches it, but no publish step existed (only supervisor-inputs + recorder-output/containment were published). **rev 7:** the **supervisor** is the sole publisher of the exact canonical signed challenge doc, `challenge_handle = SHA256(JCS({payload,sig}))`, which must exist + re-hash **before** the record is signed.
  4. **P1-4 — challenge-key trust depended on the un-started 3b-2.** Sourcing challenge public keys from the 3b-2 operator manifest was a sequencing deadlock (3b-2 cannot start until 3b-1 is GREEN + merged). **rev 7:** a **self-contained 3b-1B root-pinned `brops.challenge-key-registry.v1`** (binary-pinned challenge-root anchor in the supervisor config) resolves `challenge_key_id` in-tx; no 3b-2 dependency.
  5. **P1-5 — historical verification would break after challenge expiry.** Requiring `now ∈ [issued_at, expires_at]` at re-verification would invalidate valid completed runs once the short-lived challenge expired. **rev 7:** acceptance-time gates `now ∈ window`; durable re-verification checks the **as-of-run** predicate `issued_at ≤ requested_at ≤ expires_at` and evaluates revocation **as of `requested_at`**, so a valid historical record stays verifiable forever.
- **Next permitted action:** submit rev 7 for Architect design review; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1A** | isolated signing boundary (services + ACL socket + authoritative RunState) | ✅ **Architect Code GREEN** @ `dffd164` (CI #108, 8/8) — §3.2 |
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on rev 6; **rev 7 proposed, not yet GREEN, no code** — §3.3 |
