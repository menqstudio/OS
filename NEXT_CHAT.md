...
> **⏭️ CURRENT ACTIVE (2026-08-07): PR #62 MERGED to `main` (tip `8ac57c9`) with its Linux live-turn job RED.** The active workflow is **PR #63 · branch `fix/live-db-and-negative-reasons`** (base `main`, task T-017), which fixes it. Two defects. The durable replay ledger (new in #62) needs a real database file, and `db.path` pointed at `$LIVE`, which is root-owned 0755 because the §2.5 floor requires that of every ancestor of a pinned artifact — so the broker could not create it and every turn blocked with `reason=db_open`. The broker now gets its own 0700 state directory rather than the deployment root being loosened. **And three negative tests passed for the wrong reason:** they asserted only that the turn was `blocked`, so under that unrelated failure they printed GREEN while the property each one exists to test was never reached — the same defect class the audit has found here three times, written fresh while fixing the others. Every negative must now name the refusal it asserts. Earlier prose below is HISTORY.
