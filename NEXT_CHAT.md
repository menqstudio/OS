...
- **Exact current HEAD:** the **rev-18 (CONSOLIDATED)** design-lock candidate was pushed at **`c3cf0aea8efd83b7f7b4ebe37538517a6bc421fa`**, whose **exact-head CI is 8/8 GREEN** (both mandatory gates `Engine · governance runtime` + `Engine · signer isolation proof`, plus the `Coordination · docs consistency gate`). **The live tip may be one doc-only commit ahead of that SHA** — always resolve it yourself: `gh pr view 31 --json headRefOid -q .headRefOid` (or `git rev-parse HEAD` after `git pull`), then query GitHub Checks for THAT exact SHA (`gh api repos/menqstudio/OS/commits/<sha>/check-runs`). **GitHub Checks is the CI authority — never trust a hardcoded run number, and CI GREEN ≠ design/Architect GREEN.**
- **Where it stands:** **3b-1A** = ✅ Architect Code GREEN (`dffd164`). **3b-1B** = ❌ Architect **Design RED** — the Architect reviewed **rev 17** @ `d8a6510` (CI #121) and returned RED with **2 P0 + 3 P1** (desktop-ingress / evidence-floor / bounded-staging); **rev 18** applied every fix in place (see §3.3) via a 5-track fan-out + integrator + two fresh red-team passes. rev 18 is a **PROPOSED** design-GREEN candidate — **NOT Architect-GREEN.** **No 3b-1B / 3b-2 / 3b-3 code exists.**
- **Your next action:** submit **rev 18** (`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`, the single normative source) for Architect design review **at the exact resolved HEAD**. If RED → run the fan-out/integrator/red-team loop and revise in place; if GREEN → only then write 3b-1B code. Do **not** merge PR #31, do **not** start 3b-2/3b-3, do **not** claim Architect-GREEN, do **not** touch `NoTrustedManifest` or expose production "Verified".
...
- **Branch HEAD:** the tip of `feat/wave-3b1-isolated-signer` — the substantive content is the **3b-1B addendum (currently rev 18 — CONSOLIDATED)** plus the documentation updates; the exact tip SHA moves with each doc commit.
- **Exact current HEAD (how to resolve — GitHub is authoritative):** this doc is committed *inside* the tip, so it cannot name its own commit hash. Resolve the exact current HEAD from GitHub, not from memory: **`gh pr view 31 --json headRefOid -q .headRefOid`** (the PR head SHA) or **`git rev-parse HEAD`** after `git pull`. Then **query GitHub Checks for THAT exact SHA** (`gh api repos/menqstudio/OS/commits/<sha>/check-runs`) for the real CI — **never trust a hardcoded run number.** (Architect-reviewed HEADs, historical: rev 13 @ `415e3fd` — CI #117, rev 14 @ `18a467d` — CI #118, rev 15 @ `848f2a6` — CI #119, rev 16 @ `953f738` — CI #120, rev 17 @ `d8a6510c12192daef9053f74d674cfdb80044413` — exact-head CI #121 SUCCESS 8/8. The rev-18 candidate's current pushed HEAD is **`c3cf0aea8efd83b7f7b4ebe37538517a6bc421fa`** — its exact-head CI is **8/8 GREEN** (this is evidence, not a merge/design gate; re-query GitHub Checks for the live tip). It supersedes the first rev-18 push `fb5276c`; any later doc-only commit supersedes `c3cf0ae` in turn — resolve the live tip as above.)
- **Base `main`:** `df3c0aca80cbe4a5537a9fdd53e16e26541c9c19` (Wave 3b-0 design merged, PR #30).
- The branch contains: **3b-1A code** (Architect-GREEN, §3.2) + the **3b-1 implementation index** (`WAVE_3B1_EXECUTION_BINDING_MAP.md`, now an index that defers to the addendum) + the **3b-1B design-lock addendum** (**rev 18**, design RED, §3.3 — the single normative source for all 3b-1B contracts). It carries **no 3b-1B, 3b-2, or 3b-3 code.**
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 18 (CONSOLIDATED)** — **the single normative source for every 3b-1B contract** (artifact matrix §3, exact schemas §4 incl. the control-plane §4.10 + the new desktop→sidecar ingress §4.10(g), ms time model §1, capability profile §2 + protocol-versioning §2.2 + store ACL §2.3 + bounded ingress §2.4, durable acceptance state machine §5, atomic order + E2E §6/§6.1, verification §7 + desktop-signatures-only §7.1, authorities §8; revision history in non-normative Appendix A). History: REDs rev 6→…→11 consolidation → rev 12 → … → rev 17 → the Architect **reviewed rev 17 at HEAD `d8a6510` (exact-head CI #121 SUCCESS 8/8 — CI GREEN ≠ design GREEN), and returned Design RED with 2 P0 + 3 P1 desktop-ingress/evidence-floor/bounded-staging findings, mandating a 5-track read-only fan-out audit + one integrator + a FRESH independent red-team (NOT single-context guessing, NOT a rewrite).** **rev 18 applies those corrections in place via that fan-out; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The five rev-17 → rev-18 findings (closed in place but treat as OPEN until the Architect returns design-GREEN):**
  1. **P0-1 — the desktop→sidecar governed INGRESS frame was undefined** (the flow consumed the signed challenge document + the raw system/history/generation_config bytes "at the sidecar" by assumption; the frozen `bridge.task-request` — `additionalProperties:false`, no `challenge_doc_b64`, no top-level `protocol` — cannot carry them). **rev 18 (§2.1, §2.2, §4.10(g), §6.1):** a new **`bridge.governed-turn-submit.v1`** desktop→sidecar frame + a new Tauri `governed_turn_submit` command + the one-shot sidecar orchestrator (open→staging×3→execute→result→emit `bridge.governed-turn-result.v1`); byte formulas: `system`=raw UTF-8 + `history`=JCS reuse `brops_canonical.py`, and **`generation_config` = a closed JSON object with `JCS(object)`** — a NEW strictly-additive governed-family canonicalization (owner-mandated) that leaves the frozen 3b-1A raw-UTF-8-string `generation_config_bytes` + its `receipt.rs:1216-1219` parity fixture + `prepare_governed_turn`'s `&str` signature byte-for-byte untouched (§2.2 KEEP+ADD) + Rust↔Python JCS parity; the §2.1 authority→desktop-UI signed-document return path; new §6.1 step 0.
  2. **P0-2 — the evidence-head floor gated advance on `head_sequence`**, which is a **re-anchor/re-sign counter** (rises on an unchanged chain — the real code's stale check `bro_evidence.py:112-116` keys on it), so a legitimate re-anchor was falsely forked. **rev 18 (§7, §4.5, §4.9):** the floor keys on **`head_sequence` monotonicity + chain-content** `(event_count, last_sequence, final_event_hash)` via a **head-keyed A–E matrix** (A lower head → new `stale_evidence` reason; B same head + content diff → `evidence_fork`; C unchanged re-anchor advances only `highest_head_sequence`, NOT a fork; D longer chain accepted only if it reproduces the stored prefix by per-event hash; E every other higher-head anomaly → `evidence_fork`); a `validate_chain_detailed` helper returns the per-event hashes `validate_chain` discards; `head_sequence` demoted to `highest_head_sequence` high-water; the envelope binds `event_count`/`last_sequence`.
  3. **P1-3 — staging chunks had no minimum length and no count bound** (tiny-chunk amplification). **rev 18 (§2.4, §4.10(b/c)):** deterministic `expected_chunk_len = min(184320, declared_len − byte_count)` (else `nondeterministic_chunk`) + `MAX_STAGING_CHUNKS = 46` (`seq` 0..45, else `too_many_chunks`) + exact numeric quotas (2 turns / 6 sessions / 49 files-per-turn / 98 per-install / 17825792 bytes / 60000 ms sweep); corrected frame proof (≤2-digit seq → ≤245963 ≤262144); zero-byte artifact rule.
  4. **P1-4 — the FAILED staging session dead-ended + a state-name drift + vague publish/CHECKs.** **rev 18 (§2.4, §4.10(a/b/c)):** the per-chunk publish frozen to the exact `os.link` create-if-absent primitive (`brops_evidence_store.py:157`, O_EXCL fallback; not rename/renameat2); a `SESSION_CORRUPT` terminal contract (all later open/chunk/final ⇒ `session_corrupt`, never finalize/reuse; `session_corrupt` added to the a/b/c enums); the `INPUTS_ARTIFACT_READY`→`ARTIFACT_READY` rename resolving the drift vs the row's `INPUTS_READY`; CHECK constraints on state/`next_seq`(0..46)/`byte_count`(≥0 **and** ≤`declared_len`)/`declared_len`(≥0 **and** ≤8388608)/`artifact`(closed enum).
  5. **P1-5 — `BLOCKED` wrongly listed `UNSEEN` (no-row) as a predecessor + "the sidecar originates no reasons" was over-absolute.** **rev 18 (§5, §4.5, §4.10(f), §6.1, §7.1):** `BLOCKED` predecessors reduced to `ACCEPTED_PREPARED`/`LEASE_READY` only (pre-accept refusals create NO acceptance row — they return the §4.10(a0/a/b/c) protocol reason); the over-absolute claim reconciled to "no supervisor/signature **verdict**", with local one-shot-subprocess transport failures (spawn/socket/timeout/oversize/malformed/unexpected-exit) homed as **out-of-band Tauri Blocks** (no result, nonce not consumed) at §6.1 step 0/14, §7.1, §4.10(f).
- **Independent red-team (fresh, not the integrator):** two adversarial passes total across rev-18 — the first on the initial closure, the second (after the owner's tighter re-mandate: `generation_config` object+JCS, the head-keyed A–E floor, `stale_evidence`, and the GitHub-only continuation law) against the full working-tree diff + the real repo code. Both returned **no BLOCKER**; every P0/P1 closure verified against `brops_canonical.py`/`bro_evidence.py`/`brops_evidence_store.py`/`ai.rs` and the frozen 3b-1A family proven untouched (not in the diff); the `check_coordination` manifest gate + its tests run GREEN live (11/11). All lower-severity notes across both passes — including a `last_sequence ≥ 0`-vs-`≥ 1` DDL mismatch and a case-C "byte-identical envelope" wording bug — were **fixed before commit**.
...
- **Next permitted action:** submit rev 18 for Architect design review at the exact pushed HEAD; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on the consolidated rev 17 (@ `d8a6510`, CI #121 SUCCESS ≠ design GREEN); **rev 18 CONSOLIDATED proposed, not yet GREEN, no code** — §3.3 |
