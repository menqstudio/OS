...
> **⏭️ CURRENT ACTIVE (2026-08-06): PR #53 and #54 MERGED to `main` (tip `de38cb1`).** The active workflow is **PR #55 · branch `fix/tcb-pin-manifest-live-kit`** (base `main`, task T-017) — the second half of audit **F-10**. After #53 the §2.5 TCB integrity floor had a real `O_PATH|O_NOFOLLOW` probe and a fail-closed caller, but the live kit had no pin manifest to present, so the floor could only ever refuse for absence. This provisions the full `TCB_REQUIRED_ARTIFACTS` set from real digests of the files that genuinely serve each role, makes the `*.ipc-policy` roles real (each server loads its own root-owned `brops.ipc-policy.v1` file and refuses to serve without it), and calls the floor from `live_turn` before the governed turn runs. With this, every blocker from the 2026-08-06 independent audit is closed. **No gate flip:** `platform_governed_execution_supported()` stays `false` and `main()` keeps `UpstreamBlockedExecutor` until a SEPARATE audit passes and the Owner approves. Earlier merged-PR prose below is HISTORY.
