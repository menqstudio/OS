...
- **Exact current HEAD:** the **rev-21 (CONSOLIDATED)** design-lock candidate is committed locally (this cycle Claude does NOT push — hands the exact push + `gh pr edit` commands to Gev, see §3.3; the PR #31 body was left at rev-19 last cycle and is refreshed to rev-21 by that `gh pr edit`); the immediately-preceding **rev-20** candidate was Architect-reviewed at exact HEAD **`85240edf9bd66673d9f3e8f94e732aab155273f9`** (exact-head CI **#126** — the two mandatory gates `Engine · governance runtime` + `Engine · signer isolation proof` + the `Coordination · docs consistency gate` SUCCESS; the `Cockpit · Tauri host` job was still running at review, which does not change the verdict — CI GREEN ≠ design GREEN). **Resolve the live tip yourself** after Gev pushes: `gh pr view 31 --json headRefOid -q .headRefOid` (or `git rev-parse HEAD` after `git pull`), then query GitHub Checks for THAT exact SHA (`gh api repos/menqstudio/OS/commits/<sha>/check-runs`). **GitHub Checks is the CI authority — never trust a hardcoded run number, and CI GREEN ≠ design/Architect GREEN.**
- **Where it stands:** **3b-1A** = ✅ Architect Code GREEN (`dffd164`). **3b-1B** = ❌ Architect **Design RED** — the Architect reviewed **rev 20** @ `85240ed` (exact-head CI **#126** — mandatory gates SUCCESS; CI GREEN ≠ design GREEN), **CONFIRMED CLOSED** the rev-19 nonce/hash P0, and returned RED with a **new 1 P0 · 0 P1** on the same chain (the desktop `generation_config` hash SOURCE was contradictory — the frozen `prepare_governed_turn(&str)` hashes the raw string while 3b-1B needs the object-JCS hash, so the desktop pre-store and the authority/staging derive different `request_sha256` ⇒ Block); **rev 21** closes it in place (§4.10(g)) via a read-only real-code investigation + one integrator + a fresh independent red-team. rev 21 is a **PROPOSED** design-GREEN candidate — **NOT Architect-GREEN.** **No 3b-1B / 3b-2 / 3b-3 code exists.**
- **Your next action:** Gev pushes the local rev-21 commit + refreshes the PR #31 body; then submit **rev 21** (`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`, the single normative source) for Architect design review **at the exact resolved HEAD**. If RED → run the fan-out/integrator/red-team loop and revise in place; if GREEN → only then write 3b-1B code. Do **not** merge PR #31, do **not** start 3b-2/3b-3, do **not** claim Architect-GREEN, do **not** touch `NoTrustedManifest` or expose production "Verified". **Push/PR-write/merge are Gev's — Claude hands exact commands only (canonical mode).**
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 21 (CONSOLIDATED)** — **the single normative source for every 3b-1B contract** (artifact matrix §3, exact schemas §4 incl. the control-plane §4.10 + the desktop→sidecar ingress + preparation contract §4.10(g), ms time model §1, capability profile §2 + challenge-creation channel §2.1 + protocol-versioning §2.2 + store ACL §2.3 + bounded ingress §2.4, durable acceptance state machine §5, atomic order + E2E §6/§6.1, verification §7 + desktop-signatures-only §7.1, authorities §8; revision history in non-normative Appendix A). History: REDs rev 6→…→19 (Architect RED @ `8d3451e`, CI #125, 1 P0 · 0 P1) → rev 20 → the Architect **reviewed rev 20 at HEAD `85240ed` (exact-head CI #126 — mandatory gates SUCCESS; CI GREEN ≠ design GREEN), CONFIRMED CLOSED the rev-19 nonce/hash P0, and returned Design RED with a new 1 P0 · 0 P1 (the desktop `generation_config` hash source was contradictory — split authority between the frozen raw-string `prepare_governed_turn` and the object-JCS 3b-1B requirement), mandating a read-only real-code investigation + one integrator + a FRESH independent red-team (NOT single-context guessing, NOT a rewrite).** **rev 21 applies that correction in place; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The one rev-20 → rev-21 finding (closed in place but treat as OPEN until the Architect returns design-GREEN):** *(the rev-18 → rev-19 → rev-20 findings — orchestrator ordering, generation_config canonicalization, two-trust-model channel, nonce/`request_sha256` decoupling — are CONFIRMED CLOSED by the Architect; now history in the addendum's non-normative Appendix A.)*
  1. **P0-1 — the desktop `generation_config` hash SOURCE was still contradictory (split authority).** rev 20 requires the desktop pre-stored `IssuedRequest`, the challenge authority, and sidecar staging all use `generation_config_sha256 = SHA256(JCS(flat string→string generation_config OBJECT))`, while §2.2 KEEP+ADD freezes `prepare_governed_turn(&str)` byte-for-byte. But the merged `prepare_governed_turn` (`ai.rs:1214-1235`) hashes the **raw UTF-8 string** (`ai.rs:1231`) into `GovernedRequestContext`→`IssuedRequest`→the `receipt_challenges` pre-store, and `GOVERNED_GENERATION_CONFIG` is a plain **string** (`commands.rs:785`), not an object — so the desktop would pre-store a raw-string-based `request_sha256` while the authority/staging derive the object-JCS-based one ⇒ every legitimate turn Blocks (`handle_not_challenge`/`request_sha256` mismatch at `receipt_store.rs:322-327`), and rev 20 defined no single immutable desktop source that produces the object-JCS hash once and feeds it everywhere. **rev 21 (§4.10(g)):** adds a NEW immutable 3b-1B preparation contract **`prepare_governed_turn_v1b`** / `PreparedGovernedTurnV1B` that in ONE pass validates the flat `generation_config` **object**, computes `generation_config_jcs = JCS(object)` + its `generation_config_sha256` **once**, mints `request_nonce` once, and is the **single source** for the `receipt_challenges` pre-store `IssuedRequest`, the create-pending (A) request, the `bridge.governed-turn-submit.v1` submit frame (carrying the validated object), and the final `Expected` — so the **same** object-JCS hash flows through `receipt_challenges` → authority row → §4.1 challenge → §2.4 staging → terminal record → `Expected`, with no split authority. The frozen `prepare_governed_turn(&str)` + `GOVERNED_GENERATION_CONFIG: &str` + the `receipt.rs:1215-1219` raw-string fixture stay byte-for-byte (§2.2 KEEP+ADD). Mandatory tests: the frozen fixture's raw-string hash **≠** the object-JCS hash, and **only** the object-JCS hash appears at every 3b-1B hop.
- **Independent red-team (fresh, not the integrator):** one adversarial pass over the full rev-21 working-tree diff + the real repo code; returned **no BLOCKER** — the P0-1 closure verified against the real `ai.rs` (`prepare_governed_turn`/`PreparedGovernedTurn`/`GovernedRequestContext`), `receipt.rs` (`IssuedRequest`/`Expected`), `commands.rs` (`GOVERNED_GENERATION_CONFIG`/`issue_challenge` ordering), and `receipt_store.rs` (the pre-store + the `receipt_store.rs:322-327` mismatch gate); the frozen 3b-1A/Wave-3a preparation path proven untouched (not in the diff), and `check_coordination` + `check_capabilities` GREEN live. Any lower-severity notes were fixed before commit.
...
- **Push/PR-write mode (canonical):** this cycle Claude committed **locally only** and hands Gev the exact `git push` + `gh pr edit` commands; Gev pushes / updates the PR / merges. OWNERS gives Gev final approval/merge authority; the AI does not push or merge. **Coordination note:** the PR #31 body lagged at rev-19 after last cycle's push-only; the rev-21 handoff includes the `gh pr edit` to bring it current.
- **Next permitted action:** Gev pushes the local rev-21 commit + refreshes the PR body; then submit rev 21 for Architect design review at the exact pushed HEAD; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on the consolidated rev 20 (@ `85240ed`, CI #126 mandatory-gates SUCCESS ≠ design GREEN; rev-19 nonce/hash P0 CONFIRMED CLOSED; new 1 P0 · 0 P1 generation_config hash-source split-authority); **rev 21 CONSOLIDATED proposed, not yet GREEN, no code** — §3.3 |
