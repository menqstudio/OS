...
> **⏭️ CURRENT ACTIVE (2026-08-06): PR #54 MERGED to `main` (tip `a79c662`).** The active workflow is **PR #53 · branch `feat/windows-broker-machineproof`** (base `main`, task T-017) — the production `trusted_verified` KEYSTONE, now carrying the remediation of the 2026-08-06 independent audit. Closed on this branch: **F-01** (durable supervisor ledger; `attest-run` has no `facts` parameter), **F-23**, the **F-09** floor, **F-11**, **F-02/F-18** (per-run handles, containment evidence, and the recorder's measured evidence chain), **F-08** (the lease pins the three request digests and the launcher re-hashes the held fds 3/4/5 before exec), **F-17/F-07/F-28** (a root-owned TCB trust anchor that states its provenance — a kit-generated root can no longer report `production_verified=true` — and group-scoped store/report/socket dirs instead of `1777`), **F-06/F-13/F-14** (a self-owned trust anchor is refused; the L-4 evidence floor is a durable mark instead of `x >= x`), plus F-26/F-27/F-29/F-31/F-32/F-36/F-46. **F-10 is PARTIAL** (real probe + fail-closed caller; the live kit does not yet provision the full 22-artifact pinned set). The Linux 7-service live turn runs on every CI event and now reports honestly: `trusted_verified(production …) production_verified=false bound=true root_anchor=kit_generated`. **No gate flip:** `platform_governed_execution_supported()` stays `false` and `main()` keeps `UpstreamBlockedExecutor` until a SEPARATE audit passes and the Owner approves. Earlier merged-PR prose below is HISTORY.
