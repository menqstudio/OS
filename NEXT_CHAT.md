...
> **⏭️ CURRENT ACTIVE (2026-08-04): Phase 2 COMPLETE on `main` — PR #48/#49/#50/#51/#52 all MERGED (tip `b91f2356`, green).** All four AI surfaces (`stream_reply` main seam + `reply_in_conversation` #50 + `stream_ask` #51 + `stream_run_step` #52) now route through `ai::governed_turn`; the generic provider fallthrough is dev-only opt-in (`BROPS_ALLOW_UNGOVERNED`), fail-closed by default — no production command reaches the model provider except through the governance wall. The active workflow is now **PR #53 · branch `feat/windows-broker-machineproof`** (base `main`, task T-017, branch head `20f2fb1`, CI green): the **Windows LIVE governed-turn machine-proof** (crate `brops-win-live`) — the FULL Windows governed turn PROVEN to production `trusted_verified` over real `\\.\pipe\` named pipes with peer-SID auth (in-process CI-portable crypto chain that runs on the Linux runner too; real-pipe same-account across 3 processes; cross-account across 3 DISTINCT dedicated Windows service accounts; peer-SID gate fail-closed both directions). Additive only; no gate-logic change. **This cycle (`1bda438..20f2fb1`) additionally:** graduated production key custody (offline root-key ceremony `win_gen_root` + `CUSTODY_CEREMONY.md`; the operator's production root is TCB-pinned with its private half OFFLINE, and a SEPARATE compiled-in demonstration anchor drives the in-process proof; per-principal seeds now sealed at rest via DPAPI); gave the in-app Bro a **bounded, owner-authorized Bash coding agent** (hard `--disallowedTools` deny-list — never push/delete/install); added **chat Stop + type-while-thinking queue + 1-click copy**, a responsive hamburger drawer + 1560px wide-screen cap + lighter ambient, right-click **open-in-new-window** + route↔URL-hash sync + Esc-close; fixed the in-app "claude CLI timed out" (agent runs with `--setting-sources ""` + a 900s budget) and the i18n/nav/capability(`cancel_reply`+`open_window`)/undici(7.29.0) gaps; and **closed every finding from TWO independent adversarial audits** (the Windows-broker independent verdict is GREEN — the Owner waived the external ChatGPT/Architect audit, so three adversarial passes stand as the gate verdict) plus **group-chat speaker attribution** in the model transcript. A tagged **GitHub Release `brops-desktop-v0.1.0`** (published 2026-08-03, target `feat/windows-broker-machineproof`) ships the Windows installer (`BroPS_0.1.0_x64-setup.exe` + `BroPS_0.1.0_x64_en-US.msi`). **Post-`97483f2` this cycle** (through `20f2fb1`): supply-chain reconciliation (cryptography kept at `46.0.3` — the 50.0.0 bump broke the engine governance runtime — with 3 X.509/PKCS#7 advisories waived as the engine uses only Ed25519; undici `^7.29.0`); group-chat speaker-attribution hardening (colon-strip + roster authority guard so the renderer can't forge a `Name:` attribution + roster in the model system prompt); the Wave-3a governed-accept author annotated as a documented latent gap (P0-1, fail-closed today); and `proof::in_process_turn_output` — the governed chain can now sign+bind a REAL reply’s bytes (the wiring foundation, still fail-closed, unused by the shipped runtime). **Shipped "Verified" stays fail-closed** — `platform_governed_execution_supported()` stays false on Windows pending the remaining broker hardening (broker as its own service account, `CreateProcessAsUser` + restricted token, CNG key custody) + a separate Architect audit; `main()` still keeps `UpstreamBlockedExecutor`; the branch is NOT merged to `main`. Earlier merged-PR prose below is HISTORY.
...
| | |
|---|---|
| **Active PR / branch / task** | **PR #53** (`feat/windows-broker-machineproof`, base `main`, branch head `20f2fb1`, CI green) — the additive Windows LIVE governed-turn machine-proof for **task T-017**. Self-carrier; exact-head anchored by its PR-body `AUDIT_CANDIDATE_HEAD` marker. (PR #48, the Wave 3b design+impl consolidation, is now MERGED into `main`.) |
