...
> Tokens (validated against `config/current_state.json.status_tokens`): `CURRENT_ACTIVE_TASK: T-017` · `CURRENT_ACTIVE_WAVE: 3b-1B` · `CURRENT_PHASE0: done` · `CURRENT_DESIGN_GATE: PENDING_REAUDIT` · `CURRENT_DESIGN_PR: 31` · `CURRENT_IMPL_PR: 32` · `CURRENT_IMPL_STATE: WIP` · `CURRENT_PRODUCTION_VERIFIED: false` · `CURRENT_VERIFY_SEAM: complete` · `CURRENT_RECEIPT_PLUMBING: complete` · `CURRENT_GOVERNED_ROUNDTRIP: complete`
...
> - **PR #31** (`feat/wave-3b1-isolated-signer`, base `main`, open, **rebased onto the repaired `main` `b6c6712`**): the **3b-1A** isolated-signer boundary **code is Architect Code GREEN**; it also carries the **3b-1B rev-26 design-lock addendum**, which is **NOT Architect-GREEN** (design candidate — a fresh exact-head Architect design audit is the required next gate). PR #31 is the **self-carrier** of this snapshot; its exact audited HEAD is the live PR #31 head (recorded in the PR body + `design_gate`), verified live for state/branch/base but exempt from its own head-drift check.
> - **PR #32** (`impl/wave-3b1b-execution-binding`, base PR #31, HEAD `0e7ee1a`, **Draft/WIP**): a large **3b-1B implementation EXISTS here** (acceptance ledger, evidence-head floor, dual-key separation, recorder-owned execution, principal isolation, two-namespace store, desktop verifier). **CI-green is NOT design/audit-green**: PR #32 is **NOT an RC, NOT merge-ready**, has not passed a fresh exact-head zero-trust audit, and still needs its own rebase onto the rewritten PR #31 branch.
...
> - **Next permitted action:** rev-26's **3 preparation-P0 are now CLOSED** on this HEAD (executor-binary sha pin + non-writable TCB integrity §2.5/§4.3/§4.7; distinct-UID sidecar linchpin §2.6; Windows normative design + enforced fail-closed platform gate §0.1/§0.W) — **submit rev-26 for the Architect design audit at PR #31's exact HEAD.** rev-26's design gate is **PENDING re-audit** (the prior 3b-1B design rev was Architect-RED; rev-26 has not itself received an Architect verdict; preparation-review closure is NOT the Architect's verdict).
>
...
| | |
|---|---|
| **Next task** | **Wave 3b-1B rev-26 design audit prep** on **PR #31** (`feat/wave-3b1-isolated-signer`, rebased onto the repaired `main` `b6c6712`). Remediate rev-26's **3 preparation-P0** in `docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`: (1) pin `executor_executable_sha256` like `launcher_executable_sha256` **and** require the TCB binaries to be root-owned / non-writable by the in-scope login user; (2) specify **distinct-UID** provisioning for the desktop's own sidecar (no single-UID collapse); (3) add the **Windows** normative design + an enforced, tested **fail-closed platform gate** (not prose). Then **submit rev-26 for the Architect design audit at PR #31's exact rebased HEAD** (the Architect verdict on the exact SHA is the gate — CI-green is not audit-green). Only after 3b-1B is design-GREEN + implemented + code-audit GREEN + CI GREEN does 3b-1 merge; then 3b-2 → 3b-3 (first production `trusted_verified`). `NoTrustedManifest` stays fail-closed until the full chain is exact-head zero-trust GREEN. |
