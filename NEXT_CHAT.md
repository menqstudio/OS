...
> **⏭️ CURRENT ACTIVE (2026-08-02): Phase 2 COMPLETE on `main` — PR #48/#49/#50/#51/#52 all MERGED (tip `b91f2356`, green).** All four AI surfaces (`stream_reply` main seam + `reply_in_conversation` #50 + `stream_ask` #51 + `stream_run_step` #52) now route through `ai::governed_turn`; the generic provider fallthrough is dev-only opt-in (`BROPS_ALLOW_UNGOVERNED`), fail-closed by default — no production command reaches the model provider except through the governance wall. The active workflow is now **PR #53 · branch `feat/windows-broker-machineproof`** (base `main`, task T-017): the **Windows LIVE governed-turn machine-proof** (crate `brops-win-live`) — the FULL Windows governed turn PROVEN to production `trusted_verified` over real `\\.\pipe\` named pipes with peer-SID auth (in-process CI-portable crypto chain that runs on the Linux runner too; real-pipe same-account across 3 processes; cross-account across 3 DISTINCT dedicated Windows service accounts; peer-SID gate fail-closed both directions). Additive only; no gate-logic change. **Shipped "Verified" stays fail-closed** — `platform_governed_execution_supported()` stays false on Windows pending the remaining broker hardening (broker as its own service account, `CreateProcessAsUser` + restricted token, CNG key custody) + a separate Architect audit; `main()` still keeps `UpstreamBlockedExecutor`. Earlier merged-PR prose below is HISTORY.
