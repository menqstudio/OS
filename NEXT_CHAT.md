...
- **Branch HEAD:** the tip of `feat/wave-3b1-isolated-signer` — confirm with `git rev-parse HEAD`. The substantive content is the **3b-1B addendum (currently rev 17 — CONSOLIDATED)** plus the documentation updates; the exact tip SHA moves with each doc commit. **Always re-confirm the exact HEAD from git, and query GitHub Checks for that HEAD's real CI — never trust a hardcoded run number.** (Architect-reviewed HEADs: rev 13 @ `415e3fd` — CI #117, rev 14 @ `18a467d` — CI #118, rev 15 @ `848f2a6` — CI #119, rev 16 @ `953f73852893457fdd848e1979ebfcf05bc2f34b` — exact-head CI #120 SUCCESS 8/8.)
- **Base `main`:** `df3c0aca80cbe4a5537a9fdd53e16e26541c9c19` (Wave 3b-0 design merged, PR #30).
- The branch contains: **3b-1A code** (Architect-GREEN, §3.2) + the **3b-1 implementation index** (`WAVE_3B1_EXECUTION_BINDING_MAP.md`, now an index that defers to the addendum) + the **3b-1B design-lock addendum** (**rev 17**, design RED, §3.3 — the single normative source for all 3b-1B contracts). It carries **no 3b-1B, 3b-2, or 3b-3 code.**
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 17 (CONSOLIDATED)** — **the single normative source for every 3b-1B contract** (artifact matrix §3, exact schemas §4 incl. the control-plane §4.10, ms time model §1, capability profile §2 + protocol-versioning §2.2 + store ACL §2.3 + bounded ingress §2.4, durable acceptance state machine §5, atomic order + E2E §6/§6.1, verification §7 + desktop-signatures-only §7.1, authorities §8; revision history in non-normative Appendix A). History: REDs rev 6→…→11 consolidation → rev 12 → … → rev 16 → the Architect **reviewed rev 16 at HEAD `953f738` (exact-head CI #120 SUCCESS 8/8 — CI GREEN ≠ design GREEN), and returned Design RED with 1 P0 + 3 P1 durability/idempotency/stream-binding/state findings, mandating a 4-track read-only fan-out audit + one integrator + a FRESH independent red-team (NOT single-context guessing, NOT a rewrite).** **rev 17 applies those corrections in place via that fan-out; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The four rev-16 → rev-17 findings (closed in place but treat as OPEN until the Architect returns design-GREEN):**
  1. **P0-1 — staging chunk persistence was not crash-consistent** (a mutable append file + SQLite state with no atomic file-fsync↔DB-commit ordering; `running_sha256` is a finalized digest, not a resumable hash context — two crash cuts: DB-ahead-of-file, file-ahead-of-DB). **rev 17 (§2.4, §4.10(b/c)):** **immutable per-chunk storage** — a per-session `0700` dir; each chunk `<session_id>/<seq>.chunk` O_EXCL→fsync file→atomic link→fsync dir→**then** one `BEGIN IMMEDIATE` (re-check next_seq→insert chunk row→advance→commit)→**ACK only after commit**; 3 restart-recovery reconciliation rules; final SHA-256 recomputed from byte zero via the idempotent `EvidenceStore.publish`; `running_sha256` removed.
  2. **P1-2 — the idempotency contract contradicted itself** (§2.4 still said `seq != next_seq ⇒ refuse` while §4.10(b) split it four ways; §4.10(a) hardcoded `next_seq: 0` vs the "current cursor" reopen requirement). **rev 17 (§2.4, §4.10(a/b)):** the stale collapsed rule DELETED; the single four-way split rule; `next_seq: <int ≥ 0>` (first open 0, reopen current); the chunk reply as a discriminated union (ack⇒reason null; refused⇒non-null literal).
  3. **P1-3 — the output-read binding test was impossible + a false confidentiality claim** (the request carried only `{output_stream_id, seq}`, so the supervisor could not detect a valid other-turn token; and the spec falsely claimed the token gives confidentiality from the compromised proxy sidecar; zero-byte output was undefined; the bridge reply enum was a `<enum, superset>` placeholder). **rev 17 (§4.10(f)):** both hops carry `{output_stream_id, receipt_id, execution_attempt_id, seq}`, the supervisor compares all three (→`stream_binding_mismatch`), the desktop sources the ids from the verified §4.9 envelope; the confidentiality claim → honest threat scope (token = anti-guessing only; integrity via the signed envelope; **no confidentiality from the sidecar**; e2e encryption = future contract); a zero-byte rule (row exists; seq==0→empty,eof; seq>0→`seq_out_of_range`); the bridge reason enum literal-identical to the supervisor's five.
  4. **P1-4 — the "closed" state machine still had non-deterministic alternatives** (`EXPIRED/BLOCKED`, `RECOVERY_REQUIRED/BLOCKED`) and every refusal field was a `<enum>`/"superset" placeholder. **rev 17 (§4.7, §5, §6.1, §4.5, §4.6, §4.10):** one **deterministic** destination per condition (lease-gate → `EXPIRED`; post-EXECUTION_STARTING crash → `RECOVERY_REQUIRED`; never `BLOCKED`); a `CHECK (state IN …9…)` constraint + the deterministic state-purpose matrix; and every refusal field replaced with a **literal closed reason array** (the 25-member `GOVERNED_REFUSAL_REASONS` embedded verbatim in the relays; the exact per-message sets elsewhere).
- **Independent red-team (fresh, not the integrator):** PASS on all a–g, verifying the immutable-per-chunk ordering, the deleted stale rule, the 3-tuple binding + honest scope, and the deterministic states + literal enums against the real `brops_evidence_store.py` primitives; only two trivial naming/completeness notes, both fixed before commit.
...
- **Next permitted action:** submit rev 17 for Architect design review at the exact pushed HEAD; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on the consolidated rev 16 (@ `953f738`, CI #120 SUCCESS ≠ design GREEN); **rev 17 CONSOLIDATED proposed, not yet GREEN, no code** — §3.3 |
