...
> **⏭️ CURRENT ACTIVE (2026-08-07): PR #61 MERGED to `main` (tip `153a32f`).** The active workflow is **PR #62 · branch `remediation/round3-wave3`** (base `main`, task T-017). The headline: the chain's **replay defences were a declared test double in production**. `verify_and_accept`'s ledger — §7.1(c) receipt-id uniqueness and §7.1(d) one-time nonce consume — was `InMemoryLedger` at every production site on BOTH platforms, so the same signed receipt and the same one-time nonce were accepted again after a restart; they are a `BEGIN IMMEDIATE` CAS over SQLite now, and the old read-then-write interface (whose `record_receipt` returned `()` and so could not report a failure to record) is gone. Also: store inputs are bound to their **inodes** and re-measured immediately before `fexecve`; the lease's request pins are compared **at turn time** by the launcher against a root-owned config at a compile-time path; three further Windows twin-drift instances are closed (publish-before-validate, an existence-only chain check, an unmeasured executor image, and a PID attested by a report that observed no process); and **§1.5 step 4** policy authorization is implemented again after sitting as a defined-but-never-raised refusal reason. **Still open:** the §4.3 signed lease, the §4.7 execution receipt, §2.3 store ACLs and §7.1 freshness, plus the remaining audit findings. **No gate enabled.** Earlier prose below is HISTORY.
