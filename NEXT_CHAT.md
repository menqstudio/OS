...
> **⏭️ CURRENT ACTIVE (2026-08-02): PR #48 (Wave 3b), #49 (Windows §0.W syscall), #50 (slice 1) and #51 (slice 2 — `stream_ask` governed) MERGED to `main` (tip `1e8597c`, green).** The active workflow is now **PR #52 · branch `feat/govern-stream-run-step`** (base `main`, task T-017): Phase-2 slice 3 (final) — `stream_run_step` routed through the governed wall (reuses the held accept; the verified body becomes the step result; fail-closed). **ALL THREE ungoverned provider back doors are now closed** — no production command reaches the model provider except through the governance wall. No gate-logic change; no production `trusted_verified`; shipped "Verified" stays fail-closed. Earlier merged-PR prose below is HISTORY.
