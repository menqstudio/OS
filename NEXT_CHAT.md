...
> **⏭️ CURRENT ACTIVE (2026-08-07): PRs #59 and #60 MERGED to `main` (tip `327519c`).** The active workflow is **PR #61 · branch `remediation/round3-p1s`** (base `main`, task T-017) — round-3 remediation, waves 1 and 2. It closes the **surviving P0**: the sudoers rule placed no restriction on `governed_recorder`'s arguments, so the broker uid could steer the recorder and have that trusted identity write an authentic evidence chain for an execution the broker authored. Argv no longer steers the recorder — its paths and pins come from a root-owned policy at a compile-time path, and it re-hashes the launcher immediately before exec. Also closed: `bound` was a tautology and `production_verified` never asked WHICH root anchor verified the manifest (a demonstration root now yields `DemonstrationCustody`, not `Production`); Windows seed/config custody and the missing §2.5 floor and pipe DACL; the UI telling its owner every reply is governed when the shipped default is ungoverned; and `brops-win-live` running in no CI workflow at all. **Still open:** 20 of the audit's 23 gate-blocking findings, including the launcher's `st_dev` binding, the §4.3 signed lease, the §4.7 execution receipt, and the replay ledger being `InMemoryLedger` at every production site. Earlier prose below is HISTORY.
