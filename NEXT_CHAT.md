...
- **Branch HEAD:** the tip of `feat/wave-3b1-isolated-signer` — confirm with `git rev-parse HEAD`. The substantive content is the **3b-1B addendum (currently rev 11 — CONSOLIDATED)** plus the documentation updates; the exact tip SHA moves with each doc commit. **Always re-confirm the exact HEAD from git, never from memory.** (Architect-reviewed HEADs: rev 8 @ `59a7f04`, rev 9 @ `cb821ed`, rev 10 @ `6b1f8f4a42a3cf9f8746af971187ea91f82232fa`.)
- **Base `main`:** `df3c0aca80cbe4a5537a9fdd53e16e26541c9c19` (Wave 3b-0 design merged, PR #30).
- The branch contains: **3b-1A code** (Architect-GREEN, §3.2) + the **3b-1 implementation index** (`WAVE_3B1_EXECUTION_BINDING_MAP.md`, now an index that defers to the addendum) + the **3b-1B design-lock addendum** (**rev 11**, design RED, §3.3 — the single normative source for all 3b-1B contracts). It carries **no 3b-1B, 3b-2, or 3b-3 code.**
...
- **Machine evidence:** exact-head CI **#106…#113** and the latest **#114** (on the rev-10 HEAD) were **fully GREEN** — all **8** jobs, including both mandatory gates **`Engine · governance runtime`** and **`Engine · signer isolation proof`**. The Linux isolation job proves a **positive supervisor→signer signed round-trip BEFORE** the four same-login-user **denial** proofs, using dedicated service users. The docs/addendum commits do not touch 3b-1A code, so the `dffd164` GREEN stands; **#114 is the latest exact-head fully-GREEN run. CI GREEN ≠ design GREEN** — the 3b-1B addendum is still design RED (§3.3).
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 11 (CONSOLIDATED)** — **the single normative source for every 3b-1B contract** (artifact matrix §3, exact schemas §4, ms time model §1, closed capability profile §2, durable acceptance state machine §5, verification §7, authorities §8; revision history demoted to non-normative Appendix A). History: RED rev 6 (2 P0+3 P1) → rev 7 → RED (2 P0+2 P1) → rev 8 → RED (1 P0+3 P1) → rev 9 → RED (1 P0+1 P1) → rev 10 → the Architect **reviewed rev 10 at HEAD `6b1f8f4` (exact-head CI #114 fully GREEN — CI GREEN ≠ design GREEN), returned Design RED (3 P0 + 2 P1), and directed a one-pass consolidation.** **rev 11 is my proposed design-GREEN candidate of that consolidation; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The five rev-10 → rev-11 findings (closed by the rev-11 consolidation but treat as OPEN until the Architect returns design-GREEN):**
  1. **P0-1 — mixed timestamp units.** rev 10 compared base-lease `issued_at_epoch`/`expires_at_epoch` (epoch **seconds**, `int(time.time())`) directly to `challenge_accepted_at` (epoch **ms**). **rev 11 (§1):** one canonical **epoch-millisecond** model, every governed-turn field named `*_ms`; the base lease (`*_epoch`, seconds) is unchanged and unused by this path; one supervisor clock read; inclusive boundaries; sec-vs-ms/overflow/boundary tests.
  2. **P0-2 — the "one DB tx signs a file" claim was not actually atomic.** **rev 11 (§5):** a durable **acceptance state machine + outbox** — ledger with `UNIQUE(install_id,request_nonce)` + `UNIQUE(challenge_handle)` + `UNIQUE(execution_attempt_id)`; state enum `UNSEEN→ACCEPTED_PREPARED→LEASE_READY→EXECUTION_STARTING→EXECUTING→COMPLETED` (+ BLOCKED/FAILED/RECOVERY_REQUIRED); persist the exact canonical lease payload bytes in the commit, then idempotently sign+publish, then CAS to LEASE_READY only after the lease re-hashes/re-verifies; execution forbidden before LEASE_READY; crash recovery at every cut point; a crash mid-execution never auto-re-executes.
  3. **P0-3 — the governed-turn lease inherited code-builder overgrants.** Base task classes grant `EXECUTE_CODE`/`WRITE_FILESYSTEM`/`WRITE_REPOSITORY`; `protected_scope` was a string. **rev 11 (§2, §4.3):** a dedicated closed **`governed-model-turn-v1`** profile — `allowed_capabilities == ["INVOKE_GOVERNED_MODEL"]` only, `max_tool_calls == 0` (tool use fails closed), pinned launcher digest + model profile, and the builder-only fields (`repository`/`branch`/`worktree`/`head_sha`/`tree_identity`/`protected_scope`) removed.
  4. **P1-4 — relay schemas were only field-lists.** **rev 11 (§4.4–4.6, §8):** exact normative schemas for governed-turn sign-request / sign-result (tagged union) / bridge-result / record; bridge & sign-result echoes are **TRANSPORT-ONLY** (the desktop takes authority from verified attestation/lease/record then equality-checks the echoes); §8 references only `issue_governed_turn_lease` / `validate_governed_turn_lease`.
  5. **P1-5 — the 3b-1 map body carried stale architecture** (evidence-recorder signs the terminal record; desktop supplies `execution_attempt_id`; `run_and_sign`/`verify_passing_receipt`). **rev 11:** the map is now a concise index whose current-truth invariants match the addendum, with no re-inlined schemas.
- **Next permitted action:** submit rev 11 for Architect design review at the exact pushed HEAD; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1A** | isolated signing boundary (services + ACL socket + authoritative RunState) | ✅ **Architect Code GREEN** @ `dffd164` (exact-head CI #114, 8/8) — §3.2 |
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on rev 10 (@ `6b1f8f4`, CI #114 GREEN ≠ design GREEN); **rev 11 CONSOLIDATED proposed, not yet GREEN, no code** — §3.3 |
