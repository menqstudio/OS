...
- **Branch HEAD:** the tip of `feat/wave-3b1-isolated-signer` — confirm with `git rev-parse HEAD`. The substantive content is the **3b-1B addendum (currently rev 8)** plus the documentation updates; the exact tip SHA moves with each doc commit. **Always re-confirm the exact HEAD from git, never from memory.**
- **Base `main`:** `df3c0aca80cbe4a5537a9fdd53e16e26541c9c19` (Wave 3b-0 design merged, PR #30).
- The branch contains: **3b-1A code** (Architect-GREEN, §3.2) + the **3b-1 re-scope map** + the **3b-1B design-lock addendum** (**rev 8**, design RED, §3.3). It carries **no 3b-1B, 3b-2, or 3b-3 code.**
...
- **Machine evidence:** exact-head CI **#106, #107, #108** and the latest **#111** (on the current branch HEAD) were **fully GREEN** — all **8** jobs, including both mandatory gates **`Engine · governance runtime`** and **`Engine · signer isolation proof`**. The Linux isolation job proves a **positive supervisor→signer signed round-trip BEFORE** the four same-login-user **denial** proofs, using dedicated service users. The docs/addendum commits do not touch 3b-1A code, so the `dffd164` GREEN stands; **#111 is the latest exact-head fully-GREEN run** (supersedes the earlier transient #109 note where the Tauri job was still mid-run).
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 8**. History: the Architect returned Design RED on rev 6 (2 P0 + 3 P1); rev 7 closed those literal five, but the Architect **reviewed rev 7 and returned Design RED** on a **deeper consistency audit** (2 P0 + 2 P1). **rev 8 is my proposed closure of those four; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The four rev-7 → rev-8 blockers (addressed by rev 8 but treat as OPEN until the Architect returns design-GREEN):**
  1. **P0-1 — "trusted desktop database" is not a real Linux trust boundary.** Same-login sidecar RCE is in-scope, so a same-UID sidecar could directly edit the SQLite/store file the desktop wrote — the API oracle was closed but the direct-store-tamper path was not. **rev 8:** the pending-challenge store is **owned by a dedicated challenge-authority principal/UID** (`0700`; sidecar UID has **no r/w/list**); rows are created **only over an `SO_PEERCRED` channel admitting a UID distinct from the sidecar** (fail-closed where a platform can't separate the UIDs); Linux CI proves the **direct file/DB-mutation** denial in addition to the API-insert denial.
  2. **P0-2 — historical timestamp predicate used the wrong field.** `requested_at` is a *desktop* stamp from BEFORE the authority stamps `issued_at`, so `issued_at ≤ requested_at` can never hold and every valid run would be rejected. **rev 8:** a **supervisor-stamped `challenge_accepted_at`** (attempt-reservation instant, bound to lease + record + attestation) is the checked field — acceptance and durable re-verification require `requested_at ≤ challenge_accepted_at` and `issued_at ≤ challenge_accepted_at ≤ expires_at`, with revocation evaluated **as of `challenge_accepted_at`**.
  3. **P1-3 — challenge-key registry lacked a durable trust contract.** rev 7 defined only `keys[]` + a rotation window; a key rotated out later would make the "forever-verifiable" record unverifiable, and the record bound no registry snapshot. **rev 8:** full normative `brops.challenge-key-registry.v1` (top-level schema, `registry_epoch`/`registry_hash`/`root_key_id`, JCS signed-doc wire, same-epoch/different-hash rejection, durable anti-rollback floor, accepted-snapshot publication); the record binds `challenge_registry_handle`/`challenge_registry_epoch`/`challenge_registry_root_key_id` and re-verification resolves the key **from the stored snapshot**, so rotation never breaks history.
  4. **P1-4 — input-FD framing contradicted the size limits.** The FD table said each input FD was u32-framed at a 256 KiB cap while the ingress contract allows `history` up to 8 MiB. **rev 8:** FDs `3`/`4`/`5` are **read-only regular-file descriptors to the exact content-addressed bytes** (no length prefix — the 256 KiB frame belongs only to the IPC/ingress path), launcher-validated (regular file, `O_RDONLY`, offset 0, per-artifact size ceiling incl. history ≤ 8 MiB, store-owned inode), read to EOF.
- **Next permitted action:** submit rev 8 for Architect design review; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1A** | isolated signing boundary (services + ACL socket + authoritative RunState) | ✅ **Architect Code GREEN** @ `dffd164` (exact-head CI #111, 8/8) — §3.2 |
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on rev 7; **rev 8 proposed, not yet GREEN, no code** — §3.3 |
