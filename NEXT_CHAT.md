...
- **Branch HEAD:** the tip of `feat/wave-3b1-isolated-signer` — confirm with `git rev-parse HEAD`. The substantive content is the **3b-1B addendum (currently rev 13 — CONSOLIDATED)** plus the documentation updates; the exact tip SHA moves with each doc commit. **Always re-confirm the exact HEAD from git, and query GitHub Checks for that HEAD's real CI — never trust a hardcoded run number.** (Architect-reviewed HEADs: rev 9 @ `cb821ed`, rev 10 @ `6b1f8f4`, rev 11 @ `ac353149bb6a0c9eb0c930fba73b2c739d0229c7`, rev 12 @ `8d83246786120d5f3c6337315d904222d7991c19` — exact-head CI #116 GREEN.)
- **Base `main`:** `df3c0aca80cbe4a5537a9fdd53e16e26541c9c19` (Wave 3b-0 design merged, PR #30).
- The branch contains: **3b-1A code** (Architect-GREEN, §3.2) + the **3b-1 implementation index** (`WAVE_3B1_EXECUTION_BINDING_MAP.md`, now an index that defers to the addendum) + the **3b-1B design-lock addendum** (**rev 13**, design RED, §3.3 — the single normative source for all 3b-1B contracts). It carries **no 3b-1B, 3b-2, or 3b-3 code.**
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 13 (CONSOLIDATED)** — **the single normative source for every 3b-1B contract** (artifact matrix §3, exact schemas §4, ms time model §1 with the legacy epoch-seconds evidence exception, closed capability profile §2 + protocol-versioning §2.2 + store ACL §2.3 + bounded ingress §2.4, durable acceptance state machine §5, atomic order + E2E §6/§6.1, verification §7 + desktop-signatures-only §7.1, authorities §8; revision history demoted to non-normative Appendix A). History: REDs rev 6→7→8→9→10 → rev 11 consolidation → rev 12 surgical closure → the Architect **reviewed rev 12 at HEAD `8d83246` (exact-head CI #116 fully GREEN — CI GREEN ≠ design GREEN), and returned Design RED with 3 P0 + 3 P1 implementation-readiness findings, mandating a 6-track read-only fan-out audit + one integrator + a read-team pass (NOT single-context guessing, NOT a rewrite).** **rev 13 applies those corrections in place via that fan-out; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The six rev-12 → rev-13 findings (closed in place but treat as OPEN until the Architect returns design-GREEN):**
  1. **P0-1 — rev 12 redefined the already-GREEN 3b-1A protocol v1.** rev 12 added fields to `brops.sign-request.v1`/`brops.sign-result.v1`/`bridge.result`, which are `additionalProperties:false` and are exercised by the GREEN signer isolation positive-control (v1 requires `builder_id` + string timestamps). **rev 13 (§2.2, §4.4–4.6):** a **separate versioned governed protocol family** — `brops.governed-sign-request.v1` / `brops.governed-sign-result.v1` / `brops.governed-evidence-request.v1` / `brops.governed-result.v1` / `brops.governed-receipt-envelope.v1` / `bridge.governed-result.v1` — with its own schema files; the ratified v1 protocols are **frozen byte-for-byte and untouched**; each path refuses the other's documents.
  2. **P0-2 — exact-request-bytes bounded ingress had disappeared again.** No protocol carried the exact request bytes into the protected store under a bound. **rev 13 (§2.4):** `brops.governed-staging-open/-chunk/-final.v1` authenticated chunked upload to supervisor-owned staging; per-artifact caps (`system ≤ 256 KiB`, `history ≤ 8 MiB`, `generation_config ≤ 64 KiB`, `policy_bundle ≤ 64 KiB`, `total ≤ 8.5 MiB`); `chunk ≤ 256 KiB`, 0-based seq, dup/gap reject, streaming sha256, committed handle `==` the challenge's `*_sha256`, reuses `EvidenceStore.publish`, TTL/quota/crash cleanup, staging `0700`.
  3. **P0-3 — the desktop verification model was physically impossible + a `0700`-store contradiction.** The bridge transports only handles/echoes and the protected store is not desktop-readable, yet the record said the desktop deep-verifies; and a literal `0700` store cannot be recorder-written + signer-read. **rev 13 (§2.3, §4.6, §4.9, §6.1, §7/§7.1):** the **isolated signer's `LiveRunStateProvider` is the only deep verifier** (fetch-by-handle inside the boundary), then it **signs a canonical receipt envelope** (`brops.governed-receipt-envelope.v1`); the **desktop verifies only signatures** — signer-envelope signature + supervisor attestation + `nonce`/`receipt_id`/freshness/echoes — with **no store access, no head-floor**. Store ACL is the proven `brops-store` **group** model (setgid `2770` store dirs, `0640` artifacts, `brops-recorder` principal, signer read-only, private-key dirs `0700`), NOT literal `0700`.
  4. **P1-4 — "all governed-turn timestamps are ms" contradicted the reused evidence chain.** `bro_evidence` events/head carry `issued_at_epoch` in **seconds** via `broctl`. **rev 13 (§1, §7, matrix row 8):** a **legacy epoch-seconds exception** — evidence event/head time is seconds and is **NEVER compared to ms**; only structural bindings (`event_hash`/`sequence`/`final_event_hash`/`head_sequence`) cross in; head anti-rollback is **engine-side** (`head_sequence` vs durable `min_head_sequence` high-water), **not** a desktop floor.
  5. **P1-5 — receipt/containment/record schemas weren't exact + a `receipt_handle`↔`execution_receipt_handle` drift.** **rev 13 (§4.7/§4.7b/§4.8/§4.9, Appendix B):** four **complete** schemas (execution-receipt / containment / terminal-record / receipt-envelope); one name **`execution_receipt_handle`** everywhere; `record_handle = SHA256(JCS({payload,signature}))` defined and bound; Appendix B handle + principal + protocol matrices corrected.
  6. **P1-6 — the model output channel (FD 6) was unbounded.** **rev 13 (§4.7):** **`MAX_OUTPUT_BYTES = 8 MiB`** (`8388608`, matches the desktop's real `MAX_ASSISTANT_OUTPUT`) + read timeout + backpressure; on `MAX_OUTPUT_BYTES + 1` → stop reading, `SIGKILL` executor + cgroup teardown, **no receipt**, reasons `output_oversize`/`output_timeout`; tests at max-1/max/max+1/timeout/partial.
...
- **Next permitted action:** submit rev 13 for Architect design review at the exact pushed HEAD; revise until **design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on the consolidated rev 12 (@ `8d83246`, CI #116 GREEN ≠ design GREEN); **rev 13 CONSOLIDATED proposed, not yet GREEN, no code** — §3.3 |
