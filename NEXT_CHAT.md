...
- **Branch HEAD:** the tip of `feat/wave-3b1-isolated-signer` — confirm with `git rev-parse HEAD`. The substantive content HEAD is the **3b-1B addendum rev 6** commit; a documentation-consolidation commit sits on top. **Always re-confirm the exact HEAD from git, never from memory.**
- **Base `main`:** `df3c0aca80cbe4a5537a9fdd53e16e26541c9c19` (Wave 3b-0 design merged, PR #30).
- The branch contains: **3b-1A code** (Architect-GREEN, §3.2) + the **3b-1 re-scope map** + the **3b-1B design-lock addendum** (rev 6, RED, §3.3). It carries **no 3b-1B, 3b-2, or 3b-3 code.**
...
- **Machine evidence:** exact-head CI **#106, #107, and #108** (at `027603d`) were **fully GREEN** — all **8** jobs, including both mandatory gates **`Engine · governance runtime`** and **`Engine · signer isolation proof`**. The Linux isolation job proves a **positive supervisor→signer signed round-trip BEFORE** the four same-login-user **denial** proofs, using dedicated service users.
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), currently at **rev 6** (branch tip commit). rev 6 is my **proposed** closure of the rev-5 design RED — it is **NOT Architect-reviewed / NOT design-GREEN**. **3b-1B implementation has NOT started.**
...
- **OPEN 3b-1B design findings (raised by the Architect on rev 5; addressed by the proposed rev 6 but NOT yet Architect-GREEN — treat as OPEN until the Architect returns design-GREEN):**
  1. **P0 — challenge-authority custody.** The challenge must be signed by a **dedicated `desktop-challenge-authority`** (own service principal/SID or hardware/app-scoped key), **not** the same-login sidecar principal; sidecar key-read/list, `ptrace`, and no-`sign(payload)`-oracle denials must be **machine-proven**; it must accept only a **protected pending-challenge ID** and load the authoritative challenge itself; key id / provisioning / rotation / revocation locked.
  2. **P0 — challenge↔execution-context binding.** The signed challenge must bind **`run_id`/`task_id`** + all authoritative execution identities (or a signed immutable `challenge_id` the supervisor resolves from a protected registry); **no caller-supplied unsigned identity** may enter the terminal record.
  3. **P0 — exact executor-input delivery.** The order + mechanism must be frozen: verify challenge + staging handles → **publish/import inputs BEFORE execution** → reserve attempt + issue lease → launch the recorder with immutable handles / **read-only input FD(s)** (launcher preserves only approved input FD(s) + output FD) → executor consumes exact bytes → recorder publishes/​signs output+containment → supervisor verifies + signs the terminal record.
  4. **P1 — bounded ingress = ONE normative mechanism.** Pick one Linux production mechanism and freeze auth, chunk/frame size, per-artifact + total caps, sequence/dup/out-of-order rules, session/challenge binding, temp-file `O_EXCL`, hash+size verify, atomic commit, expiry/cleanup, quota, crash behavior. Windows may stay explicitly **fail-closed** until its broker is separately audited.
  5. **P1 — durable authenticated-challenge evidence.** The terminal record must durably bind the original signed challenge (`challenge_handle`, `challenge_key_id`, signed-doc handle/bytes, `challenge_issued_at`, `challenge_expires_at`, `request_sha256`) so `LiveRunStateProvider` can independently re-verify it.
- **Next permitted action:** revise the addendum until it is **Architect design-GREEN**. Only then may 3b-1B **code** be written.
...
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED**, rev 6 proposed, **no code** — §3.3 |
