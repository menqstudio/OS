...
- **Exact current HEAD:** the **rev-22 (CONSOLIDATED)** design-lock candidate is **pushed at HEAD `a84ee12dd8b7033729e8c7aa4628b18e23e02939`**, whose **exact-head CI is 8/8 GREEN** (both mandatory gates + coordination + capability + the Cockpit jobs), and the PR #31 body is refreshed to rev-22. Push/PR-write are now Owner-delegated to Claude (§3.3). The immediately-preceding **rev-21** candidate was Architect-reviewed at exact HEAD **`a05629b7179e9ee87f315e5ac8452e88c8f4f89a`** (exact-head CI **#127** — the two mandatory gates + coordination gate SUCCESS; `Cockpit · Tauri host` still running at review, which does not change the verdict — CI GREEN ≠ design GREEN). **Resolve the live tip yourself:** `gh pr view 31 --json headRefOid -q .headRefOid` (or `git rev-parse HEAD` after `git pull`), then query GitHub Checks for THAT exact SHA (`gh api repos/menqstudio/OS/commits/<sha>/check-runs`). **GitHub Checks is the CI authority — never trust a hardcoded run number, and CI GREEN ≠ design/Architect GREEN.**
- **Where it stands:** **3b-1A** = ✅ Architect Code GREEN (`dffd164`). **3b-1B** = ❌ Architect **Design RED** — the Architect reviewed **rev 21** @ `a05629b` (exact-head CI **#127** — mandatory gates SUCCESS; CI GREEN ≠ design GREEN), **CONFIRMED CLOSED** the rev-20 generation_config hash-source split, and returned RED with a **new 1 P0 · 0 P1** on the same chain (the `PreparedGovernedTurnV1B` lifecycle was severed at the Tauri/frontend boundary — rev 21 made submit a separate frontend-invoked command re-accepting raw fields after the pre-store, so the single immutable object couldn't survive to submit/`Expected` without a webview re-serialize ⇒ Block); **rev 22** closes it in place (§4.10(g), §6.1) via a read-only real-code investigation + one integrator + a fresh independent red-team. rev 22 is a **PROPOSED** design-GREEN candidate — **NOT Architect-GREEN.** **No 3b-1B / 3b-2 / 3b-3 code exists.**
- **Your next action:** rev 22 is pushed (HEAD `a84ee12`) and the PR #31 body is refreshed to rev-22; submit **rev 22** (`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`, the single normative source) for Architect design review **at the exact resolved HEAD**. If RED → run the fan-out/integrator/red-team loop and revise in place; if GREEN → only then write 3b-1B code. Do **not** merge PR #31, do **not** start 3b-2/3b-3, do **not** claim Architect-GREEN, do **not** touch `NoTrustedManifest` or expose production "Verified". **Push/PR-write are Owner-delegated to Claude (see §3.3); merge + final design approval remain the Owner's (Gev). The AI never merges.**
...
- **File:** [`docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md`](./docs/design/WAVE_3B1B_EXECUTION_BINDING_ADDENDUM.md), now at **rev 22 (CONSOLIDATED)** — **the single normative source for every 3b-1B contract** (artifact matrix §3, exact schemas §4 incl. the control-plane §4.10 + the desktop→sidecar ingress + preparation contract + the one `governed_turn_execute` command §4.10(g), ms time model §1, capability profile §2 + challenge-creation channel §2.1 + protocol-versioning §2.2 + store ACL §2.3 + bounded ingress §2.4, durable acceptance state machine §5, atomic order + E2E §6/§6.1, verification §7 + desktop-signatures-only §7.1, authorities §8; revision history in non-normative Appendix A). History: REDs rev 6→…→20 (Architect RED @ `85240ed`, CI #126, 1 P0 · 0 P1) → rev 21 → the Architect **reviewed rev 21 at HEAD `a05629b` (exact-head CI #127 — mandatory gates SUCCESS; CI GREEN ≠ design GREEN), CONFIRMED CLOSED the rev-20 generation_config hash-source split, and returned Design RED with a new 1 P0 · 0 P1 (the `PreparedGovernedTurnV1B` lifecycle was severed at the Tauri/frontend boundary — submit was a separate frontend-invoked command re-accepting raw fields after the pre-store), mandating a read-only real-code investigation + one integrator + a FRESH independent red-team (NOT single-context guessing, NOT a rewrite).** **rev 22 applies that correction in place; it is NOT yet Architect-reviewed / NOT design-GREEN.** **3b-1B implementation has NOT started.**
...
- **The one rev-21 → rev-22 finding (closed in place but treat as OPEN until the Architect returns design-GREEN):** *(the rev-18 → … → rev-20 findings — orchestrator ordering, generation_config canonicalization, two-trust-model channel, nonce/`request_sha256` decoupling, generation_config hash-source split — are CONFIRMED CLOSED by the Architect; now history in the addendum's non-normative Appendix A.)*
  1. **P0-1 — the `PreparedGovernedTurnV1B` lifecycle was severed at the Tauri/frontend boundary.** rev 21 required ONE immutable Rust `PreparedGovernedTurnV1B` to be the single source for pre-store, authority request, submit, and final `Expected`, but defined **submit as a separate frontend-invoked Tauri command** re-accepting raw fields `{task_id, challenge_doc_b64, system, history, generation_config}` **after** challenge creation + `receipt_challenges` pre-store — with no defined way for the same in-process object to reach submit/`Expected` without a webview re-serialize/reconstruct. So the submit's object-JCS bytes could differ from the already-pre-stored `request_sha256` ⇒ fail-closed Block again; the implementer was left to pick an undefined workaround (re-take raw fields / accept frontend claims / an undefined cache / reconstruct a second object) — the first two destroy the single-source guarantee, the last two lack ownership/TTL/retry/crash/one-time-consume semantics. **rev 22 (§4.10(g), §6.1):** submit is **no longer** a frontend command; 3b-1B adds **exactly one** frontend-exposed governed Tauri command **`governed_turn_execute(raw UI inputs)`** (mirroring the merged single-backend `stream_reply`, `commands.rs:794/844-935`) that, in ONE backend execution owning a single in-process `PreparedGovernedTurnV1B`, runs prepare-v1b once → `receipt_challenges` pre-store → authority create-pending + issue → the **internal** helper `governed_turn_submit_prepared(&prepared, challenge_document)` (NOT a Tauri command) → the internal output-pull loop → final `Expected` from the **same** `&prepared`. Nothing round-trips the webview after prepare; `PreparedGovernedTurnV1B` fields are **private** with read-only accessors and no mutable public copies; before submit the backend asserts `SHA256(generation_config_jcs) == context.generation_config_sha256` **and** `issued_request().request_sha256() == the pre-stored receipt_challenges.request_sha256`. A frontend-exposed raw-field submit command is **forbidden**; the only permitted alternative is a server-side opaque `prepared_turn_id` state machine (bounded store, TTL, closed `PREPARED→CHALLENGED→SUBMITTED→FINALIZED`, one-time-consume). Mandatory test: a frontend that mutates `config`/`system`/`history` after `governed_turn_execute` begins cannot reach submit or alter the pre-stored request.
- **Independent red-team (fresh, not the integrator):** one adversarial pass over the full rev-22 working-tree diff + the real repo code; returned **no BLOCKER** — the P0-1 closure verified against the merged single-backend `stream_reply` shape (`commands.rs:794`, prepare→pre-store→execute→verify at `commands.rs:844-935`) and `ai.rs::governed_engine`/`generate_handler!`; the frozen 3b-1A/Wave-3a path proven untouched (not in the diff), and `check_coordination` + `check_capabilities` GREEN live. Any lower-severity notes were fixed before commit.
...
- **Push/PR-write mode (Owner-delegated 2026-07-25 — supersedes the earlier canonical "hand-commands" mode):** the Owner (Gev) has **delegated `git push` + `gh pr edit` to Claude** ("all pushes on you as well, I am checking only"); Claude now **commits, pushes, and updates the PR #31 body directly** each cycle, and Gev reviews. **Merge + final design approval remain the Owner's (Gev) per OWNERS; the AI NEVER merges** and never claims Architect/design-GREEN. (The Architect design review itself is still ChatGPT's; the audit is the gate.) The rev-22 commit `a84ee12` is pushed, its exact-head CI is **8/8 GREEN**, and the PR #31 body is refreshed to rev-22.
- **Next permitted action:** submit rev 22 @ pushed HEAD `a84ee12` (resolve the live tip from GitHub) for Architect design review; if RED, run the fan-out/integrator/red-team loop, revise in place, commit+push+refresh-PR-body, and re-submit; if GREEN, only then write 3b-1B **code** (after Owner "go").
...
| **3b-1B** | authoritative execution→receipt binding (design-lock) | ❌ **design RED** — Architect RED on the consolidated rev 21 (@ `a05629b`, CI #127 mandatory-gates SUCCESS ≠ design GREEN; rev-20 hash-source P0 CONFIRMED CLOSED; new 1 P0 · 0 P1 PreparedGovernedTurnV1B lifecycle severed at Tauri boundary); **rev 22 CONSOLIDATED proposed, not yet GREEN, no code** — §3.3 |
