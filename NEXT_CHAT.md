...
- **The five rev-17 → rev-18 findings (closed in place but treat as OPEN until the Architect returns design-GREEN):**
  1. **P0-1 — the desktop→sidecar governed INGRESS frame was undefined** (the flow consumed the signed challenge document + the raw system/history/generation_config bytes "at the sidecar" by assumption; the frozen `bridge.task-request` — `additionalProperties:false`, no `challenge_doc_b64`, no top-level `protocol` — cannot carry them). **rev 18 (§2.1, §2.2, §4.10(g), §6.1):** a new **`bridge.governed-turn-submit.v1`** desktop→sidecar frame + a new Tauri `governed_turn_submit` command + the one-shot sidecar orchestrator (open→staging×3→execute→result→emit `bridge.governed-turn-result.v1`); byte formulas LOCKED to the shipped `brops_canonical.py` (system=raw UTF-8, history=JCS, generation_config=raw UTF-8) + Rust↔Python JCS parity; the §2.1 authority→desktop-UI signed-document return path; new §6.1 step 0.
  2. **P0-2 — the evidence-head floor gated advance on `head_sequence`**, which is a **re-anchor/re-sign counter** (rises on an unchanged chain — `bro_evidence.py:63-67`), so a legitimate re-anchor was falsely forked. **rev 18 (§7, §4.9):** the floor keys on **chain-content** `(event_count, last_sequence, final_event_hash)` via a **5-case matrix** (case 3 same-length same-hash + higher head_sequence = idempotent re-anchor, NOT a fork); a `validate_chain_detailed` helper returns the per-event hashes `validate_chain` discards; `head_sequence` demoted to `highest_head_sequence` high-water; the envelope binds `event_count`/`last_sequence`.
...
- **Independent red-team (fresh, not the integrator):** ran an adversarial pass against the rev-18 diff + the real repo code — **no BLOCKER**; every P0/P1 substantive closure verified (byte formulas vs `brops_canonical.py`, the chain-content floor + case-3 regression guard vs `bro_evidence.py`, the P1-3 arithmetic, the `os.link` primitive, the state/transport reconciliation, STOP gates). Six lower-severity citation/edge-case accuracy notes (zero-byte `n_chunks` formula, a nonexistent `run_governed_turn` anchor, a JCS/raw-UTF-8 justification, the `bridge.task-request` required-set quote, a case-4 0-based index, a test citation) were **all fixed before commit**.
