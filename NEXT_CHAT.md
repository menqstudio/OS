...
> **⏭️ CURRENT ACTIVE (2026-08-02): PR #48 (Wave 3b + cockpit) AND PR #49 (Windows §0.W syscall layer) are MERGED to `main` (main tip `aad6809`, green).** The active workflow is now **PR #50 · branch `feat/govern-ai-surfaces-design`** (base `main`, task T-017): Phase-2 "govern the AI surfaces" — the design proposal + slice 1 (`reply_in_conversation` now routes through the governed wall, fail-closed; production Blocks with no message; `ai::generate` demoted to a dev-only fallthrough). `stream_ask`/`stream_run_step` deferred (need core work). No gate-logic change; no production `trusted_verified`; shipped "Verified" stays fail-closed. Everything below about `feat/cockpit-pages`/PR #48 and `feat/windows-broker-syscall`/PR #49 is now HISTORY (merged).
