...
| **Audit** | **Round 1 = RED** (4 blockers on `24869eb`, PR #26) — **RESOLVED**: (1) challenge `request_sha256` NOT-NULL + hex, loaded and compared in-tx to `expected.request.request_sha256()`; (2) staged decoded evidence (exact envelope bytes + signature + key_id + receipt_id) on bad-sig/bind-failure; (3) `message_id` `ON DELETE SET NULL` so accepted evidence survives conversation/message deletion (link nulls, replay ledger kept); (4) nested-tx invocation rejected + explicit COMMIT-failure rollback. Awaiting **re-audit on the new pushed HEAD**. |
