...
> **⏭️ CURRENT ACTIVE (2026-08-02): PR #48 (Wave 3b + cockpit), #49 (Windows §0.W syscall), and #50 (Phase-2 slice 1 — `reply_in_conversation` governed) are MERGED to `main` (tip `f2b844e`, green).** The active workflow is now **PR #51 · branch `feat/govern-stream-ask`** (base `main`, task T-017): Phase-2 slice 2 — `stream_ask` routed through the governed wall (new conversation-less held-answer accept in receipt_store + migration 0016; the verified answer is stashed under a one-time result_id; fail-closed). **Two of three ungoverned provider back doors now closed.** `stream_run_step` + tightening `stream_reply`'s fallthrough remain. No gate-logic change; no production `trusted_verified`; shipped "Verified" stays fail-closed. Everything below about the earlier merged PRs is HISTORY.
