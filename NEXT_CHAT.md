...
> **⏭️ CURRENT ACTIVE (2026-08-06): the remediation audit returned 🔴 RED a SECOND time.** Two PRs are open against `main` @ `2debb71`: **PR #56** (round-1 response — closes the P0: the supervisor no longer attests an `output_handle` it never observed) and **PR #57 · branch `remediation/audit-red-round2`** (round-2 response — the active workflow, task T-017). #57 closes the **P1** where the Windows signing seeds sat on disk as plaintext until first read (`attest.seed` + `signer.seed` are the two production keys `verify_and_accept` checks, so reading them skipped the chain entirely rather than defeating it); the fact that **F-08's enforcement had zero tests** — the four cited tests covered the lease parser, so deleting the check kept the suite green, and CI now runs a NEGATIVE live turn that tampers with a pinned input and requires a refusal; **52 tests in the production crates that no CI job ran**; and a **built-in placeholder being shown as a model answer**. Merge #56 first. Still open: the Windows proof script rejected by its own provisioner, `bound` as a tautology, `production_verified` not asking which root anchor verified the manifest, the inert `FILE_FLAG_FIRST_PIPE_INSTANCE`, and the Windows kit's missing §2.5 and anti-rollback floors. **No gate flip:** `platform_governed_execution_supported()` stays `false`. Earlier prose below is HISTORY.
