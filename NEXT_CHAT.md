...
> **⏭️ CURRENT ACTIVE (2026-08-06): PRs #56 and #57 MERGED to `main` (tip `e81f50c`).** Both rounds of the remediation audit have been responded to: the **P0** (the supervisor attesting an `output_handle` it never observed) is closed on both platforms, as are the seed-custody **P1**, F-08's untested enforcement, the 52 tests no CI job ran, and a placeholder being shown as a model answer. The active workflow is **PR #58 · branch `docs/ledger-accuracy-after-round2`** (base `main`, task T-017) — docs only: the ledger's status cells and a new legend, because three rounds have now found a ✅ overclaimed, so ✅ is reserved for what an INDEPENDENT audit confirmed and ◑ means the Builder's unverified claim. **Still open** from round 2: the Windows proof script rejected by its own provisioner, `bound` as a tautology, `production_verified` not asking which root anchor verified the manifest, the inert `FILE_FLAG_FIRST_PIPE_INSTANCE`, and the Windows kit's missing §2.5 and anti-rollback floors. **No gate flip:** `platform_governed_execution_supported()` stays `false`. Earlier prose below is HISTORY.
